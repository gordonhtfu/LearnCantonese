...
import com.blackberry.email.ui.compose.FromAddressSpinnerAdapter;
...
public class FromAddressSpinner extends Spinner implements OnItemSelectedListener {
    ...
    private ReplyFromAccount mSelectedAccount;
    ...
    private void selectCurrentAccount() {
        ...
        for (ReplyFromAccount acct : mReplyFromAccounts) {
            if (TextUtils.equals(mSelectedAccount.name, acct.name)
                    && TextUtils.equals(mSelectedAccount.address, acct.address)) {
        ...
        if (action == ComposeActivity.COMPOSE || action == ComposeActivity.EDIT_DRAFT) {
            ...
            if (refMessage != null) {
                ...
                for (Account account : accounts) {
                    if (ComposeUtils.getAccountID(account) == refMessage.mAccountId) {
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        ...
        // TODO: Do not key off ReplyFromAccount.name b/11292541
        if (!selection.name.equals(mSelectedAccount.name)) {
            mSelectedAccount = selection;
