...
import com.blackberry.email.provider.UIProvider.AttachmentColumns;
import com.blackberry.email.provider.UIProvider.AttachmentContentValueKeys;
...
public class AttachmentActionHandler {
    ...
    private void startDownloadingAttachment(
        ...
        // Only retrigger a download once for a message that is already downloading
        if ((attachment.state != AttachmentState.DOWNLOADING) || !mStartedDownload) {
    ...
    public void shareAttachment() {
        ...
        final Uri uri = UtilsEx.normalizeUri(mAttachment.contentUri);
        intent.putExtra(Intent.EXTRA_STREAM, uri);
        intent.setType(UtilsEx.normalizeMimeType(mAttachment.getContentType()));
