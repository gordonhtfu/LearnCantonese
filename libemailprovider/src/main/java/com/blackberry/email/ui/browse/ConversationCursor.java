...
//import com.blackberry.mail.ui.ConversationListFragment;
...
import com.blackberry.email.provider.UIProvider.ConversationListQueryParameters;
import com.blackberry.email.provider.UIProvider.ConversationOperations;
import com.blackberry.email.utils.DrawIdler;
import com.blackberry.email.utils.ThreadSafeCursorWrapper;
import com.blackberry.email.utils.UtilsEx;
//import com.blackberry.mail.utils.NotificationActionUtils;
//import com.blackberry.mail.utils.NotificationActionUtils.NotificationAction;
//import com.blackberry.mail.utils.NotificationActionUtils.NotificationActionType;
...
public final class ConversationCursor implements Cursor, ConversationCursorOperationListener,
    ...
    private static class UnderlyingCursorWrapper extends ThreadSafeCursorWrapper
        ...
        private class CacheLoaderTask extends AsyncTask<Void, Void, Void> {
            ...
            public Void doInBackground(Void... param) {
                try {
                    UtilsEx.traceBeginSection("backgroundCaching");
        ...
        public UnderlyingCursorWrapper(Cursor result) {
            ...
            final int count;
            UtilsEx.traceBeginSection("blockingCaching");
        ...
        private void notifyConversationUIPositionChange() {
            UtilsEx.notifyCursorUIPositionChange(this, getPosition());
        }
    ...
    private UnderlyingCursorWrapper doQuery(boolean withLimit) {
        ...
        UtilsEx.traceBeginSection("query");
    ...
    public Conversation getConversation(Folder folder) {
        if (this.getPosition() < 0 && this.getCount() > 0) {
            this.moveToFirst();
        }
        Conversation conv = getConversation();
        if (conv != null) {

            if (conv.messageListUri == null) {
                conv.messageListUri = this.qUri;
                conv.uri = conv.messageListUri;
            }
            // add the folder as a new folderlist
            if(folder != null){
                ArrayList<Folder> folders = new ArrayList<Folder>();
                folders.add(folder);
                FolderList fl = FolderList.copyOf(folders);
    
                conv.setRawFolders(fl);
            }
        }
