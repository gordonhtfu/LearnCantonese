...
import com.blackberry.common.content.ObjectCursorLoader;
...
import com.blackberry.email.ui.browse.ConversationViewHeader.ConversationViewHeaderCallbacks;
...
public abstract class AbstractConversationViewFragment extends Fragment implements
    ...
    protected MessagingService mSvc;
    ...
    protected void setBaseUri() {
        ...
        mBaseUri = "x-thread://" + (mAccount != null ? mAccount.getEmailAddress().hashCode() : "") + "/" + mConversation.id;
    }
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        mWebViewClient.setAccount(mAccount);
        mSvc = new MessagingService(Long.valueOf(mAccount.uri.getPathSegments().get(1)), getContext());
    }
    ...
    public void onPrepareOptionsMenu(Menu menu) {
        ...
        UtilsEx.setMenuItemVisibility(menu, R.id.show_original, supportsMessageTransforms() &&
    ...
    protected void timerMark(String msg) {
        if (isUserVisible()) {
            UtilsEx.sConvLoadTimer.mark(msg);
        }
    }
    ...
    protected void onConversationSeen() {
        ...
        if (!mSuppressMarkingViewed) {
            ...
            if (!mConversation.isViewed() || (cursor != null && !cursor.isConversationRead())) {
                mConversation.markViewed();
                // Mark the messages in the conversation read in the provider.
                markMessagesRead();
        ...
        if(activity.getListHandler() != null){
            activity.getListHandler().onConversationSeen();
        }
    }

    protected abstract void markMessagesRead();
    ...
    private static class MessageLoader extends ObjectCursorLoader<ConversationMessage> {
        ...
        public MessageLoader(Context c, Uri messageListUri) {
            super(c, messageListUri, MessageContract.Message.DEFAULT_PROJECTION,
                    MessageContract.Message.DELETED + "=?", new String[] {"0"},
                    MessageContract.Message.TIMESTAMP + " DESC", ConversationMessage.FACTORY);
        }
