...
public class ConversationViewState implements Parcelable {
    ...
    public boolean isUnread(MessageValue m) {
        final MessageViewState mvs = mMessageViewStates.get(m.mEntityUri);
        return (mvs != null && !mvs.read);
    }

    public void setReadState(MessageValue m, boolean read) {
        MessageViewState mvs = mMessageViewStates.get(m.mEntityUri);
        if (mvs == null) {
            mvs = new MessageViewState();
        }
        mvs.read = read;
        mMessageViewStates.put(m.mEntityUri, mvs);
    }

    public boolean getShouldShowImages(MessageValue m) {
        final MessageViewState mvs = mMessageViewStates.get(m.mEntityUri);
        return (mvs != null && mvs.showImages);
    }

    public void setShouldShowImages(MessageValue m, boolean showImages) {
        MessageViewState mvs = mMessageViewStates.get(m.mEntityUri);
        if (mvs == null) {
            mvs = new MessageViewState();
        }
     ...
     */
    public Integer getExpansionState(MessageValue m) {
        final MessageViewState mvs = mMessageViewStates.get(m.mEntityUri);
        return (mvs == null ? null : mvs.expansionState);
    }

    public void setExpansionState(MessageValue m, int expansionState) {
        MessageViewState mvs = mMessageViewStates.get(m.mEntityUri);
        if (mvs == null) {
            mvs = new MessageViewState();
        }
    ...
    public boolean contains(MessageValue m) {
        return mMessageViewStates.containsKey(m.mEntityUri);
    }
