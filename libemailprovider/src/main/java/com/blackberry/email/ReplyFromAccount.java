...
public class ReplyFromAccount implements Serializable {
    ...
    public static ReplyFromAccount deserialize(Account account, JSONObject json) {
        ...
        try {
            Uri uri = Utility.getValidUri(json.getString(BASE_ACCOUNT_URI));
    ...
    public static boolean matchesAccountOrCustomFrom(Account account, String possibleCustomFrom,
        ...
        if (tokens != null && tokens.length > 0) {
            String parsedFromAddress = Utility.normalizeEmailAddress(tokens[0].getAddress());
            if (TextUtils.equals(Utility.normalizeEmailAddress(account.getEmailAddress()),
                    parsedFromAddress)) {
                return true;
            }
            for (ReplyFromAccount replyFromAccount : replyFromAccounts) {
                if (TextUtils.equals(Utility.normalizeEmailAddress(replyFromAccount.address),
