...
public class Address implements Parcelable {
    ...
    public static synchronized Address getEmailAddress(String rawAddress) {
        ...
        if (tokens.length > 0) {
            ...
            name = tokenizedName != null ? Utility.convertHtmlToPlainText(tokenizedName.trim())
                    .toString() : "";
            address = Utility.convertHtmlToPlainText(tokens[0].getAddress()).toString();
        } else {
            name = "";
            address = rawAddress == null ?
                    "" : Utility.convertHtmlToPlainText(rawAddress).toString();
        }
    ...
    public String toString() {
        if (mName != null && !mName.equals(mAddress)) {
            if (mName.matches(".*[\\(\\)<>@,;:\\\\\".\\[\\]].*")) {
                return Utility.ensureQuotedString(mName) + " <" + mAddress + ">";
            } else {
