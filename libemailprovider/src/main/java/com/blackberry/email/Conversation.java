...
import com.blackberry.email.ui.ConversationCursorLoader;
...
public class Conversation implements Parcelable {
    ...
    public Conversation(Cursor cursor) {
        if (cursor != null) {
            ...
            // This is a bit dirty but the underlaying cursor will throw an
            // exception
            // if the value is null - which I now allow it to be null
            try {
                String strUri = cursor.getString(UIProvider.CONVERSATION_URI_COLUMN);
                if (strUri != null) {
    ...
    private static ConversationInfo readConversationInfo(Cursor cursor) {
        ...
        response.setClassLoader(ConversationInfo.class.getClassLoader());
    ...
    private static FolderList readRawFolders(Cursor cursor) {
        ...
        response.setClassLoader(FolderList.class.getClassLoader());
        if (response.containsKey(key)) {
