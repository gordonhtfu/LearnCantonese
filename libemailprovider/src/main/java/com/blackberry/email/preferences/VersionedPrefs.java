...
//import com.blackberry.mail.MailIntentService;
...
public abstract class VersionedPrefs {
    ...
    protected VersionedPrefs(final Context context, final String sharedPrefsName) {
        ...
        if (!hasMigrationCompleted()) {
            BasePreferenceMigrator migrator = PreferenceMigratorHolder.createPreferenceMigrator();
            if(migrator != null){
                final boolean migrationComplete = migrator.performMigration(context, oldVersion, CURRENT_VERSION_NUMBER);
    ...
    protected void notifyBackupPreferenceChanged() {
    	//@TODO add back after refactor **MFL
