...
import com.blackberry.common.Api;
...
public class EmailServiceUtils {
    /**
     * Ask a service to kill its process. This is used when an account is
     * deleted so that no background thread that happens to be running will
     * continue, possibly hitting an NPE or other error when trying to operate
    ...
    public static void startRemoteServices(Context context) {
        for (EmailServiceInfo info : getServiceInfoList(context)) {
            if (info.intentAction != null) {
                final Intent serviceIntent = getServiceIntent(info);
                LogUtils.i(Logging.LOG_TAG, "Starting service: " + info.name);
                context.startService(serviceIntent);
            }
        }
    }
    ...
    public static void setRemoteServicesLogging(Context context, int debugBits) {
        for (EmailServiceInfo info : getServiceInfoList(context)) {
            if (info.intentAction != null) {
                EmailServiceProxy service = EmailServiceUtils.getService(
                        context, info.protocol);
                if (service != null) {
                    try {
    ...
    /**
     * Holder of service information (currently just name and class/intent); if
     * there is a class member, this is a (local, i.e. same process) service;
     * otherwise, this is a remote service
     */
    public static class EmailServiceInfo {
        ...
        public boolean offerLookbackCount;
        public int defaultLookbackCount;
    ...
    public static EmailServiceInfo getServiceInfoForAccount(Context context,
            long accountId) {
        String protocol = Account.getProtocol(context, accountId);
        return getServiceInfo(context, protocol);
    }

    public static EmailServiceInfo getServiceInfo(Context context,
            String protocol) {
        return getServiceMap(context).get(protocol);
    }

    public static Collection<EmailServiceInfo> getServiceInfoList(
            Context context) {
        return getServiceMap(context).values();
    }

    private static void finishAccountManagerBlocker(
     ...
     *            Our {@link Context}.
     * @param account
     *            The {@link Account} we're adding.
     * @param email
     *            Whether the user wants to sync email on this account.
     * @param calendar
     *            Whether the user wants to sync calendar on this account.
     * @param contacts
     *            Whether the user wants to sync contacts on this account.
     * @param callback
     *            A callback for when the AccountManager is done.
    ...
    public static AccountManagerFuture<Bundle> setupAccountManagerAccount(final Context context,
            ...
            final boolean contacts, final boolean tasks, final boolean notes,
            final AccountManagerCallback<Bundle> callback) {
        final Bundle options = new Bundle(5);
        final HostAuth hostAuthRecv = HostAuth.restoreHostAuthWithId(context,
                account.mHostAuthKeyRecv);
        if (hostAuthRecv == null) {
            return null;
        }
        // Set up username/password
        options.putString(OPTIONS_USERNAME, account.mEmailAddress);
        options.putString(OPTIONS_PASSWORD, hostAuthRecv.mPassword);
        options.putBoolean(OPTIONS_CONTACTS_SYNC_ENABLED, contacts);
        options.putBoolean(OPTIONS_CALENDAR_SYNC_ENABLED, calendar);
        options.putBoolean(OPTIONS_EMAIL_SYNC_ENABLED, email);
        options.putBoolean(OPTIONS_TASKS_SYNC_ENABLED, tasks);
        options.putBoolean(OPTIONS_NOTES_SYNC_ENABLED, notes);
    ...
    public static void updateAccountManagerType(Context context,
        ...
        final Cursor c = resolver.query(Account.CONTENT_URI,
                Account.CONTENT_PROJECTION,
                AccountColumns.EMAIL_ADDRESS + "=?",
                new String[] { amAccount.name }, null);
        // That's odd, isn't it?
        if (c == null)
            return;
        try {
            if (c.moveToNext()) {
                ...
                account.restore(c);
                final HostAuth hostAuth = HostAuth.restoreHostAuthWithId(
                        context, account.mHostAuthKeyRecv);
                if (hostAuth == null) {
                    return;
                }
                ...
                LogUtils.w(Logging.LOG_TAG, "Converting " + amAccount.name
                        + " to " + newProtocol);

                final ContentValues accountValues = new ContentValues();
                int oldFlags = account.mFlags;

                // Mark the provider account incomplete so it can't get
                // reconciled away
                ...
                final Uri accountUri = ContentUris.withAppendedId(
                        Account.CONTENT_URI, account.mId);
                resolver.update(accountUri, accountValues, null, null);

                // Change the HostAuth to reference the new protocol; this has
                // to be done before
                ...
                resolver.update(ContentUris.withAppendedId(
                        HostAuth.CONTENT_URI, hostAuth.mId), hostValues, null,
                        null);
                LogUtils.w(Logging.LOG_TAG, "Updated HostAuths");

                try {
                    // Get current settings for the existing AccountManager
                    // account
                    boolean email = ContentResolver.getSyncAutomatically(
                            amAccount, MessageContract.AUTHORITY);
                    if (!email) {
                        // Try our old provider name
                        email = ContentResolver.getSyncAutomatically(amAccount,
                                "com.blackberry.email.provider");
                    }
                    ...
                    final boolean tasks = ContentResolver.getSyncAutomatically(amAccount,
                            TaskContract.AUTHORITY);
                    final boolean notes = ContentResolver.getSyncAutomatically(amAccount,
                            NoteContract.AUTHORITY);
                    ...
                    try {
                        calendarSyncKey = SyncStateContract.Helpers.get(
                                client,
                                asCalendarSyncAdapter(SyncState.CONTENT_URI,
                                        amName, oldType),
                    ...
                    AccountManagerFuture<?> amFuture = setupAccountManagerAccount(context, account,
                            email, calendar, contacts, tasks, notes, null);

                    finishAccountManagerBlocker(amFuture);
                    LogUtils.w(Logging.LOG_TAG,
                            "Created new AccountManager account");

                    final String accountType = protocolMap
                            .get(hostAuth.mProtocol + "_type");
                    // Move calendar and contacts data from the old account to
                    // the new one.
                    // We must do this before deleting the old account or the
                    // data is lost.
                    moveCalendarData(context.getContentResolver(), amName,
                            oldType, accountType);
                    moveContactsData(context.getContentResolver(), amName,
                            oldType, accountType);

                    // Delete the AccountManager account
                    amFuture = AccountManager.get(context).removeAccount(
                            amAccount, null, null);
                    finishAccountManagerBlocker(amFuture);
                    LogUtils.w(Logging.LOG_TAG,
                            "Deleted old AccountManager account");

                    // Restore sync keys for contacts/calendar

                    if (accountType != null && calendarSyncKey != null
                            && calendarSyncKey.length != 0) {
                        client = context.getContentResolver()
                                .acquireContentProviderClient(
                                        CalendarContract.CONTENT_URI);
                        try {
                            SyncStateContract.Helpers.set(
                                    client,
                                    asCalendarSyncAdapter(
                                            SyncState.CONTENT_URI, amName,
                                            accountType),
                                    new android.accounts.Account(amName,
                                            accountType), calendarSyncKey);
                            LogUtils.w(Logging.LOG_TAG, "Set calendar key...");
                        } catch (RemoteException e) {
                            LogUtils.w(Logging.LOG_TAG,
                                    "Set calendar key FAILED");
                        } finally {
                            client.release();
                        }
                    }
                    if (accountType != null && contactsSyncKey != null
                            && contactsSyncKey.length != 0) {
                        client = context.getContentResolver()
                                .acquireContentProviderClient(
                                        ContactsContract.AUTHORITY_URI);
                        try {
                            SyncStateContract.Helpers.set(client,
                                    ContactsContract.SyncState.CONTENT_URI,
                                    new android.accounts.Account(amName,
                                            accountType), contactsSyncKey);
                            LogUtils.w(Logging.LOG_TAG, "Set contacts key...");
                        } catch (RemoteException e) {
                            LogUtils.w(Logging.LOG_TAG,
                                    "Set contacts key FAILED");
                        }
                    }
    ...
    private static void moveCalendarData(final ContentResolver resolver,
            final String name, final String oldType, final String newType) {
        final Uri oldCalendars = Calendars.CONTENT_URI
                .buildUpon()
                .appendQueryParameter(CalendarContract.CALLER_IS_SYNCADAPTER,
                        "true")
                .appendQueryParameter(Calendars.ACCOUNT_NAME, name)
                .appendQueryParameter(Calendars.ACCOUNT_TYPE, oldType).build();

        // Update this calendar to have the new account type.
        final ContentValues values = new ContentValues();
        values.put(CalendarContract.Calendars.ACCOUNT_TYPE, newType);
        resolver.update(oldCalendars, values, Calendars.ACCOUNT_NAME
                + "=? AND " + Calendars.ACCOUNT_TYPE + "=?", new String[] {
                name, oldType });
    }

    private static void moveContactsData(final ContentResolver resolver,
            final String name, final String oldType, final String newType) {
        final Uri oldContacts = RawContacts.CONTENT_URI
                .buildUpon()
     ...
     */
    private static Map<String, EmailServiceInfo> getServiceMap(
            final Context context) {
        synchronized (sServiceMapLock) {
            /**
             * We cache localized strings here, so make sure to regenerate the
             * service map if the locale changes
             */
            if (sServiceMap == null) {
                sOldConfiguration.setTo(context.getResources()
                        .getConfiguration());
            }

            final int delta = sOldConfiguration.updateFrom(context
                    .getResources().getConfiguration());

            if (sServiceMap != null
                    && !Configuration.needNewResources(delta,
                            ActivityInfo.CONFIG_LOCALE)) {
                return sServiceMap;
            }

            final ImmutableMap.Builder<String, EmailServiceInfo> builder = ImmutableMap
                    .builder();
            ...
            try {
                ...
                while ((xmlEventType = xml.next()) != XmlResourceParser.END_DOCUMENT) {
                    if (xmlEventType == XmlResourceParser.START_TAG
                        ...
                        final TypedArray ta = res.obtainAttributes(xml,
                                R.styleable.EmailServiceInfo);

                        info.protocol = ta
                                .getString(R.styleable.EmailServiceInfo_protocol);
                        info.accountType = ta
                                .getString(R.styleable.EmailServiceInfo_accountType);
                        info.name = ta
                                .getString(R.styleable.EmailServiceInfo_name);
                        info.hide = ta.getBoolean(
                                R.styleable.EmailServiceInfo_hide, false);
                        final String klass = ta
                                .getString(R.styleable.EmailServiceInfo_serviceClass);
                        info.intentAction = ta
                                .getString(R.styleable.EmailServiceInfo_intent);
                        info.intentPackage = ta
                                .getString(R.styleable.EmailServiceInfo_intentPackage);
                        info.defaultSsl = ta.getBoolean(
                                R.styleable.EmailServiceInfo_defaultSsl, false);
                        info.port = ta.getInteger(
                                R.styleable.EmailServiceInfo_port, 0);
                        info.portSsl = ta.getInteger(
                                R.styleable.EmailServiceInfo_portSsl, 0);
                        info.offerTls = ta.getBoolean(
                                R.styleable.EmailServiceInfo_offerTls, false);
                        info.offerCerts = ta.getBoolean(
                                R.styleable.EmailServiceInfo_offerCerts, false);
                        info.offerLocalDeletes = ta.getBoolean(
                                R.styleable.EmailServiceInfo_offerLocalDeletes,
                                false);
                        info.defaultLocalDeletes = ta
                                .getInteger(
                                        R.styleable.EmailServiceInfo_defaultLocalDeletes,
                                        Account.DELETE_POLICY_ON_DELETE);
                        info.offerPrefix = ta
                                .getBoolean(
                                        R.styleable.EmailServiceInfo_offerPrefix,
                                        false);
                        info.usesSmtp = ta.getBoolean(
                                R.styleable.EmailServiceInfo_usesSmtp, false);
                        info.usesAutodiscover = ta.getBoolean(
                                R.styleable.EmailServiceInfo_usesAutodiscover,
                                false);
                        info.offerLookback = ta.getBoolean(
                                R.styleable.EmailServiceInfo_offerLookback,
                                false);
                        info.defaultLookback = ta.getInteger(
                                R.styleable.EmailServiceInfo_defaultLookback,
                                SyncWindow.SYNC_WINDOW_3_DAYS);
                        info.offerLookbackCount = ta
                                .getBoolean(
                                        R.styleable.EmailServiceInfo_offerLookbackCount,
                                        false);
                        info.defaultLookbackCount = ta
                                .getInteger(
                                        R.styleable.EmailServiceInfo_defaultLookbackCount,
                                        SyncWindow.SYNC_WINDOW_COUNT20);
                        info.syncChanges = ta
                                .getBoolean(
                                        R.styleable.EmailServiceInfo_syncChanges,
                                        false);
                        info.syncContacts = ta.getBoolean(
                                R.styleable.EmailServiceInfo_syncContacts,
                                false);
                        info.syncCalendar = ta.getBoolean(
                                R.styleable.EmailServiceInfo_syncCalendar,
                                false);
                        info.syncTasks = ta.getBoolean(
                                R.styleable.EmailServiceInfo_syncTasks, false);
                        info.syncNotes =
                                ta.getBoolean(R.styleable.EmailServiceInfo_syncNotes, false);
                        info.offerAttachmentPreload = ta
                                .getBoolean(
                                        R.styleable.EmailServiceInfo_offerAttachmentPreload,
                                        false);
                        info.syncIntervalStrings = ta
                                .getTextArray(R.styleable.EmailServiceInfo_syncIntervalStrings);
                        info.syncIntervals = ta
                                .getTextArray(R.styleable.EmailServiceInfo_syncIntervals);
                        info.defaultSyncInterval = ta
                                .getInteger(
                                        R.styleable.EmailServiceInfo_defaultSyncInterval,
                                        15);
                        info.inferPrefix = ta
                                .getString(R.styleable.EmailServiceInfo_inferPrefix);
                        info.offerLoadMore = ta.getBoolean(
                                R.styleable.EmailServiceInfo_offerLoadMore,
                                false);
                        info.offerMoveTo = ta
                                .getBoolean(
                                        R.styleable.EmailServiceInfo_offerMoveTo,
                                        false);
                        info.requiresSetup = ta.getBoolean(
                                R.styleable.EmailServiceInfo_requiresSetup,
                                false);
                        info.dav = ta.getBoolean(
                                R.styleable.EmailServiceInfo_dav, false);

                        // Must have either "class" (local) or "intent" (remote)
                        if (klass != null) {
                            try {
                                // noinspection unchecked
                                info.klass = (Class<? extends Service>) Class
                                        .forName(klass);
                            } catch (ClassNotFoundException e) {
                                throw new IllegalStateException(
                                        "Class not found in service descriptor: "
                                                + klass);
                            }
                        }
    ...
    private static Uri asCalendarSyncAdapter(Uri uri, String account,
            String accountType) {
        return uri
                .buildUpon()
                .appendQueryParameter(CalendarContract.CALLER_IS_SYNCADAPTER,
    ...
    class NullService implements IEmailService {
        ...
        public void startSync(long mailboxId, boolean userRequest,
                int deltaMessageCount) throws RemoteException {
        }
        ...
        @Override
        public void loadAttachment(final IEmailServiceCallback cb,
        ...
        @Override
        public boolean renameFolder(long accountId, String oldName,
        ...
        @Override
        public void deleteAccountPIMData(long accountId, final String emailAddress)
                throws RemoteException {
        }
        ...
        @Override
        public int searchMessages(long accountId, SearchParams params,
                long destMailboxId) throws RemoteException {
            return 0;
        }
