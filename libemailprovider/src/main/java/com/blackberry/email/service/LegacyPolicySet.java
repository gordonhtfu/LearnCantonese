...
public class LegacyPolicySet {
    ...
    public static Policy flagsToPolicy(long flags) {
        ...
        try {
            policy.setPasswordMode((int) ((flags & PASSWORD_MODE_MASK) >> PASSWORD_MODE_SHIFT));
            if (policy.getPasswordMode() > Policy.PASSWORD_MODE_NONE) {
                policy.setPasswordMinLength((int) ((flags & PASSWORD_LENGTH_MASK) >> PASSWORD_LENGTH_SHIFT));
                policy.setPasswordMaxFails((int) ((flags & PASSWORD_MAX_FAILS_MASK) >> PASSWORD_MAX_FAILS_SHIFT));
                policy.setPasswordHistory((int) ((flags & PASSWORD_HISTORY_MASK) >> PASSWORD_HISTORY_SHIFT));
                policy.setPasswordExpirationDays((int) ((flags & PASSWORD_EXPIRATION_MASK) >> PASSWORD_EXPIRATION_SHIFT));
                policy.setMaxScreenLockTime((int) ((flags & SCREEN_LOCK_TIME_MASK) >> SCREEN_LOCK_TIME_SHIFT));
            }
            ...
            // TODO Carl - I don't think we support this legacy storage so just ingore
        }
        return policy;
    }
}
