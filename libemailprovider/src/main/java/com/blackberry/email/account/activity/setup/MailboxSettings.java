...
public class MailboxSettings extends PreferenceActivity {
     ...
     * @param pref a ListPreference to be set up
     */
    public static void setupLookbackCountPreferenceOptions(final Context context,
            final ListPreference pref) {
        final Resources resources = context.getResources();
        // Load the complete list of entries/values
        CharSequence[] entries;
        CharSequence[] values;
        entries = resources.getTextArray(R.array.account_settings_mail_count_entries);
        values = resources.getTextArray(R.array.account_settings_mail_count_values);
        // Set up the preference
        pref.setEntries(entries);
        pref.setEntryValues(values);
        pref.setSummary(pref.getEntry());
