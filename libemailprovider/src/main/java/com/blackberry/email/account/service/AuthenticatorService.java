...
public class AuthenticatorService extends Service {
    ...
    public static final String OPTIONS_TASK_SYNC_ENABLED = "tasks";
    public static final String OPTIONS_NOTE_SYNC_ENABLED = "notes";
        ...
        public Bundle addAccount(AccountAuthenticatorResponse response, String accountType,
            ...
            if (options != null && options.containsKey(OPTIONS_PASSWORD)
                ...
                if (options.containsKey(OPTIONS_CONTACTS_SYNC_ENABLED)) {
                    ...
                    ContentResolver.setSyncAutomatically(account, ContactsContract.AUTHORITY, syncContacts);
                }
                ...
                if (options.containsKey(OPTIONS_CALENDAR_SYNC_ENABLED)) {
                    ...
                    ContentResolver.setSyncAutomatically(account, CalendarContract.AUTHORITY, syncCalendar);
                }

                // Set up task syncing, if appropriate
                if (options.containsKey(OPTIONS_TASK_SYNC_ENABLED)) {
                    boolean syncTasks = options.getBoolean(OPTIONS_TASK_SYNC_ENABLED);
                    ContentResolver.setIsSyncable(account, TaskContract.AUTHORITY, 1);
                    ContentResolver.setSyncAutomatically(account, TaskContract.AUTHORITY, syncTasks);
                }

                // Set up note syncing, if appropriate
                if (options.containsKey(OPTIONS_NOTE_SYNC_ENABLED)) {
                    boolean syncNotes = options.getBoolean(OPTIONS_NOTE_SYNC_ENABLED);
                    ContentResolver.setIsSyncable(account, NoteContract.AUTHORITY, 1);
                    ContentResolver.setSyncAutomatically(account, NoteContract.AUTHORITY, syncNotes);
                }
                ...
                ContentResolver.setIsSyncable(account, MessageContract.AUTHORITY, 1);
                ContentResolver.setSyncAutomatically(account, MessageContract.AUTHORITY, syncEmail);
