...
public class SetupData implements Parcelable {
    ...
    public static final String EXTRA_SETUP_DATA = "com.blackberry.email.setupdata";

    // NORMAL is the standard entry from the Email app; EAS and POP_IMAP are
    // used when entering via
    // Settings -> Accounts
    ...
    // The following two modes are used to "pop the stack" and return from the
    // setup flow. We
    // either return to the caller (if we're in an account type flow) or go to
    // the message list
    ...
    // Mode bits for AccountSetupCheckSettings, indicating the type of check
    // requested
    ...
    private boolean mCalendarFound = true;
    private boolean mContactsFound = true;
    ...
    public boolean getCalendarFound() {
        return mCalendarFound;
    }

    public boolean getContactsFound() {
        return mContactsFound;
    }

    public void setCalendarFound(boolean calendarFound) {
        mCalendarFound = calendarFound;
    }

    public void setContactsFound(boolean contactsFound) {
        mContactsFound = contactsFound;
    }
    ...
    public static final Parcelable.Creator<SetupData> CREATOR = new Parcelable.Creator<SetupData>() {
        @Override
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(mCalendarFound ? 1 : 0);
        dest.writeInt(mContactsFound ? 1 : 0);
    ...
    public SetupData(Parcel in) {
        ...
        mCalendarFound = in.readInt() == 1;
        mContactsFound = in.readInt() == 1;
    }
