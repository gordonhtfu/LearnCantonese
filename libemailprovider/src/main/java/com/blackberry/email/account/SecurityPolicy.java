...
public class SecurityPolicy {
    ...
    public int getInactiveReasons(Policy policy) {
        ...
        if (isActiveAdmin()) {
            ...
            if (policy.getPasswordMinLength() > 0) {
                if (dpm.getPasswordMinimumLength(mAdminName) < policy.getPasswordMinLength()) {
                    LogUtils.w(TAG, "Password does not meet minumum length");
                    reasons |= INACTIVE_NEED_PASSWORD;
                }
            }
            try {
                if (policy.getPasswordMode() > 0) {
                    if (dpm.getPasswordQuality(mAdminName) < policy.getDPManagerPasswordQuality()) {
                        LogUtils.w(TAG, "Password quality is not sufficient, password mode: %d", policy.getPasswordMode());
                        reasons |= INACTIVE_NEED_PASSWORD;
                    }
                    if (!dpm.isActivePasswordSufficient()) {
                        LogUtils.w(TAG, "Password quality - active password is not sufficient");
                        reasons |= INACTIVE_NEED_PASSWORD;
                    }
                }
            } catch (IllegalStateException e) {
                // This policy is not valid since the password mode has not been set
                reasons |= INACTIVE_NEED_CONFIGURATION;
            }
            if (policy.getMaxScreenLockTime() > 0) {
                // Note, we use seconds, dpm uses milliseconds
                if (dpm.getMaximumTimeToLock(mAdminName) > policy.getMaxScreenLockTime() * 1000) {
                    LogUtils.w(TAG, "Password issue = max screen lock time is not sufficient");
                    reasons |= INACTIVE_NEED_CONFIGURATION;
                }
            }
            if (policy.getPasswordExpirationDays() > 0) {
                ...
                if (currentTimeout == 0
                    ...
                    LogUtils.w(TAG, "Password issue - expiration days: current timeout: %d, policy timeout: %d", currentTimeout,
                ...
                if (expired) {
                    LogUtils.w(TAG, "Password issue - password has expired");
                    reasons |= INACTIVE_NEED_PASSWORD;
                }
            }
            if (policy.getPasswordHistory() > 0) {
                if (dpm.getPasswordHistoryLength(mAdminName) < policy.getPasswordHistory()) {
                    // There's no user action for changes here; this is just a configuration change
                    LogUtils.w(TAG, "Password issue - password history");
                    reasons |= INACTIVE_NEED_CONFIGURATION;
                }
            }
            if (policy.getMinimumNonLetterChars() > 0) {
                if (dpm.getPasswordMinimumNonLetter(mAdminName) < policy.getMinimumNonLetterChars()) {
                    LogUtils.w(TAG, "Password issue, min non-letter chars required: %d, policy value: %d",
                            dpm.getPasswordMinimumNonLetter(mAdminName), policy.getMinimumNonLetterChars());
                    reasons |= INACTIVE_NEED_PASSWORD;
                }
            }
            if (policy.mRequireEncryption) {
                ...
                if (encryptionStatus != DevicePolicyManager.ENCRYPTION_STATUS_ACTIVE) {
                    LogUtils.w(TAG, "Password issue - device encryption not enabled");
                    reasons |= INACTIVE_NEED_ENCRYPTION;
                }
            }
            if (policy.mDontAllowCamera && !dpm.getCameraDisabled(mAdminName)) {
                LogUtils.w(TAG, "Password issue - camera enabled");
                reasons |= INACTIVE_NEED_CONFIGURATION;
            }
            // password failures are counted locally - no test required here
            // no check required for remote wipe (it's supported, if we're the admin)

            if (!policy.mUnsupportedList.isEmpty()) {
                LogUtils.w(TAG, "Password issue - there are unsupported policies");
    ...
    public void setActivePolicies() {
        ...
        if (aggregatePolicy == Policy.NO_POLICY) {
            ...
            dpm.setPasswordMinimumLength(mAdminName, aggregatePolicy.getPasswordMinLength());
            // screen lock time
            dpm.setMaximumTimeToLock(mAdminName, aggregatePolicy.getMaxScreenLockTime() * 1000);
            // local wipe (failed passwords limit)
            dpm.setMaximumFailedPasswordsForWipe(mAdminName, aggregatePolicy.getPasswordMaxFails());
            ...
            dpm.setPasswordHistoryLength(mAdminName, aggregatePolicy.getPasswordHistory());
            ...
            dpm.setPasswordMinimumNonLetter(mAdminName, aggregatePolicy.getMinimumNonLetterChars());
            // Device capabilities
    ...
    public static void setAccountHoldFlag(Context context, long accountId, boolean newState) {
        ...
        if (account != null) {
            ...
            if (newState) {
                ...
                NotificationController.getInstance(context).showSecurityNeededNotification(account,true);
            }
        }
    }
    ...
    public void policiesRequired(long accountId) {
        ...
        if (policy.mUnsupportedList.isEmpty()) {
            NotificationController.getInstance(mContext).showSecurityNeededNotification(account, false);
        } else {
            NotificationController.getInstance(mContext).showSecurityUnsupportedNotification(account);
        }
    }
    ...
    public static void setAccountPolicy(Context context, Account account, Policy policy,
        ...
        if (policy != null) {
            policy.normalize(context.getResources());
    ...
    private static void syncAccount(final Context context, final Account account) {
        ...
        extras.putBoolean(Policy.POLICY_IS_ACTIVE, true);
        ContentResolver.requestSync(amAccount, MessageContract.AUTHORITY, extras);
    ...
    public void setAccountPolicy(long accountId, Policy policy, String securityKey) {
        ...
        if (!policy.mUnsupportedList.isEmpty()) {
            ...
            if (policyChanged) {
                ...
                NotificationController.getInstance(mContext).showSecurityChangedNotification( account);
            } else {
            ...
            NotificationController.getInstance(mContext).showSecurityNeededNotification(account,true);
        }
    ...
    private void onPasswordExpiring(Context context) {
        ...
        if (!expired) {
            ...
            if (wiped) {
                NotificationController.getInstance(mContext).showPasswordExpiredNotification(nextExpiringAccountId);
            }
        }
    }
    ...
    public static class PolicyAdmin extends DeviceAdminReceiver {
        ...
        public void onEnabled(Context context, Intent intent) {
            EmailBroadcastProcessorService.processDevicePolicyMessage(context, DEVICE_ADMIN_MESSAGE_ENABLED);
        }
        ...
        public void onDisabled(Context context, Intent intent) {
            EmailBroadcastProcessorService.processDevicePolicyMessage(context, DEVICE_ADMIN_MESSAGE_DISABLED);
        }
        ...
        public void onPasswordChanged(Context context, Intent intent) {
            EmailBroadcastProcessorService.processDevicePolicyMessage(context, DEVICE_ADMIN_MESSAGE_PASSWORD_CHANGED);
        }
        ...
        public void onPasswordExpiring(Context context, Intent intent) {
            EmailBroadcastProcessorService.processDevicePolicyMessage(context,DEVICE_ADMIN_MESSAGE_PASSWORD_EXPIRING);
        }
    }
}
