...
import com.blackberry.dav.service.IDavCheckSettings;
import com.blackberry.email.mail.MessagingException;
import com.blackberry.email.mail.Sender;
...
public class AccountCheckSettingsFragment extends Fragment {
    ...
    private final static String DAV_CHECK_SETTINGS_INTENT = "davCheckSettings";
    ...
    private final static int STATE_CHECK_CALENDAR = 9;
    private final static int STATE_CHECK_CONTACTS = 10;
    private int mState = STATE_START;
    private SetupData mSetupData;
    private IDavCheckSettings mDavService = null;
    private Object mBinderLock =  new Object();
    private boolean mDavServiceConnected = false;
    ...
    private ServiceConnection mDavServiceConnection = new ServiceConnection() {

        @Override
        public void onServiceDisconnected(ComponentName name) {
            mDavService = null;
            mDavServiceConnected = false;
        }

        @Override
        public void onServiceConnected(ComponentName name, IBinder service) {
            mDavService = IDavCheckSettings.Stub.asInterface(service);
            synchronized (mBinderLock) {
                mBinderLock.notifyAll();
            }
            mDavServiceConnected = true;
        }
    };
            ...
            switch (newState) {
                ...
                case STATE_AUTODISCOVER_RESULT:
                    ...
                    if (autoDiscoverResult != null) {
                        LogUtils.i(LogUtils.TAG, "Auto discover returned server address %s", autoDiscoverResult.mAddress);
        ...
        protected MessagingException doInBackground(Void... params) {
            try {
                ...
                if ((mMode & SetupData.CHECK_INCOMING) != 0) {
                    ...
                    mAccount.mCapabilities = bundle
                            .getLong(EmailServiceProxy.VALIDATE_BUNDLE_ACCOUNT_CAPABILITIES);
                    ...
                    if (resultCode == MessagingException.SECURITY_POLICIES_REQUIRED) {
                        ...
                        final Policy policy = bundle.getParcelable(
                                EmailServiceProxy.VALIDATE_BUNDLE_POLICY_SET);
                        final String[] data = policy.getUnsupportedPolicyStrings(mContext.getResources());
                ...
                if (info.dav) {
                    try {
                        boolean serviceAvailable = mContext.bindService(
                                new Intent(DAV_CHECK_SETTINGS_INTENT),
                                mDavServiceConnection, Context.BIND_AUTO_CREATE);
                        if (!mDavServiceConnected && serviceAvailable) {
                            try {
                                synchronized (mBinderLock) {
                                    mBinderLock.wait();
                                }
                            } catch (InterruptedException e) {
                                LogUtils.e(Logging.LOG_TAG, "Failed to find dav service: %s", e);
                                mSetupData.setCalendarFound(false);
                                mSetupData.setContactsFound(false);
                            }
                        } else if (!serviceAvailable) {
                            LogUtils.w(TAG, "DAV services are not installed");
                            mSetupData.setCalendarFound(false);
                            mSetupData.setContactsFound(false);
                        }
                        if (mDavService != null && serviceAvailable) {
                            publishProgress(STATE_CHECK_CALENDAR);
                            mSetupData.setCalendarFound(mDavService.hasCalendar(
                                    mAccount.mHostAuthRecv.mAddress,
                                    mAccount.mHostAuthRecv.mLogin,
                                    mAccount.mHostAuthRecv.mPassword));
                            publishProgress(STATE_CHECK_CONTACTS);
                            mSetupData.setContactsFound(mDavService.hasContacts(
                                    mAccount.mHostAuthRecv.mAddress,
                                    mAccount.mHostAuthRecv.mLogin,
                                    mAccount.mHostAuthRecv.mPassword));
                        } else if (serviceAvailable) {
                            LogUtils.w(Logging.LOG_TAG, "DAV Services was not found");
                            mSetupData.setCalendarFound(false);
                            mSetupData.setContactsFound(false);
                        }
                    } catch (RemoteException e) {
                        LogUtils.e(Logging.LOG_TAG, "Failed to find calendars/contacts: %s", e);
                        mSetupData.setCalendarFound(false);
                        mSetupData.setContactsFound(false);
                    }
                    mContext.unbindService(mDavServiceConnection);
                }
        ...
        private String getProgressString(int progress) {
            ...
            switch (progress) {
                ...
                case STATE_CHECK_OUTGOING:
                    ...
                    break;
                case STATE_CHECK_CALENDAR:
                    stringId = R.string.account_setup_check_settings_calendar_msg;
                    break;
                case STATE_CHECK_CONTACTS:
                    stringId = R.string.account_setup_check_settings_contacts_msg;
                    break;
            }
