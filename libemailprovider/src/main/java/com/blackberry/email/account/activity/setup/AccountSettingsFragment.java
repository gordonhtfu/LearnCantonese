...
import com.blackberry.email.account.ui.settings.SettingsUtils;
...
public class AccountSettingsFragment extends PreferenceFragment
    ...
    private static final String PREFERENCE_DOWNLOAD_BODY_WHILE_ROAMING = "account_download_body_while_roaming";
    ...
    private ListPreference mSyncCount;
    private CheckBoxPreference mAccountBackgroundAttachments;
    private CheckBoxPreference mAccountDownloadBodiesWhileRoaming;
    ...
    private com.blackberry.email.Account mUiAccount;
    ...
    public interface Callback {
        ...
        public void onEditQuickResponses(com.blackberry.email.Account account);
    ...
    private static class EmptyCallback implements Callback {
        ...
        @Override public void onEditQuickResponses(com.blackberry.email.Account account) {}
    ...
    private class LoadAccountTask extends AsyncTask<Long, Void, Map<String, Object>> {
        ...
        protected void onPostExecute(Map<String, Object> map) {
            if (!isCancelled()) {
                ...
                mUiAccount = (com.blackberry.email.Account) map.get(UI_ACCOUNT_KEY);
                if (account == null) {
                    mSaveOnExit = false;
                    mCallback.abandonEdit();
                } else {
    ...
    /**
     * Loads settings that are dependent on a {@link com.blackberry.mail.providers.Account}, which
    ...
    private void loadSettingsOffMainThread() {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                // @TODO put back **MFL
//                final AccountPreferences accountPreferences = new AccountPreferences(mContext, mUiAccount.getEmailAddress());
//                NotificationUtils.moveNotificationSetting(accountPreferences, mInboxFolderPreferences);
    ...
    private void loadSettings() {
        ...
        if (info.syncContacts || info.syncCalendar || info.syncTasks || info.syncNotes) {
            // This account allows syncing of contacts and/or calendar and/or tasks and/or notes, so we will always have
            // separate preferences to enable or disable syncing of email, contacts, tasks, notes, and calendar.
        ...
        } else if (info.offerLookbackCount) {
            mSyncCount = new ListPreference(mContext);
            mSyncCount.setTitle(R.string.account_setup_options_mail_count_label);
            mSyncCount.setValue(String.valueOf(mAccount.getSyncLookback()));

            MailboxSettings.setupLookbackCountPreferenceOptions(mContext, mSyncCount);

            // Must correspond to the hole in the XML file that's reserved.
            mSyncCount.setOrder(2);
            mSyncCount.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
                @Override
                public boolean onPreferenceChange(Preference preference, Object newValue) {
                    final String summary = newValue.toString();
                    int index = mSyncCount.findIndexOfValue(summary);
                    mSyncCount.setSummary(mSyncCount.getEntries()[index]);
                    mSyncCount.setValue(summary);
                    preferenceChanged(preference.getKey(), newValue);
                    return false;
                }
            });
            dataUsageCategory.addPreference(mSyncCount);
        }
...
//            sentPreference.setIntent(i);
        ...
        mAccountDownloadBodiesWhileRoaming = (CheckBoxPreference)
                findPreference(PREFERENCE_DOWNLOAD_BODY_WHILE_ROAMING);
        mAccountDownloadBodiesWhileRoaming.setChecked(
            0 != (mAccount.getFlags() & Account.FLAGS_DOWNLOAD_BODY_WHILE_ROAMING));
        mAccountDownloadBodiesWhileRoaming.setOnPreferenceChangeListener(this);
...
*/
        ...
        mSyncTasks = (CheckBoxPreference) findPreference(PREFERENCE_SYNC_TASKS);
        mSyncNotes = (CheckBoxPreference) findPreference(PREFERENCE_SYNC_NOTES);
        if (info.syncContacts || info.syncCalendar || info.syncTasks || info.syncNotes) {
            ...
            if (info.syncTasks) {
                mSyncTasks.setChecked(ContentResolver
                        .getSyncAutomatically(androidAcct, TaskContract.AUTHORITY));
                mSyncTasks.setOnPreferenceChangeListener(this);
            } else {
                mSyncTasks.setChecked(false);
                mSyncTasks.setEnabled(false);
            }
            if (info.syncNotes) {
                mSyncNotes.setChecked(ContentResolver
                        .getSyncAutomatically(androidAcct, NoteContract.AUTHORITY));
                mSyncNotes.setOnPreferenceChangeListener(this);
            } else {
                mSyncNotes.setChecked(false);
                mSyncNotes.setEnabled(false);
            }
            ...
            dataUsageCategory.removePreference(mSyncTasks);
            dataUsageCategory.removePreference(mSyncNotes);
        }
    }
    ...
    private void saveSettings() {
        ...
        newFlags |= mAccountDownloadBodiesWhileRoaming.isChecked() ?
                Account.FLAGS_DOWNLOAD_BODY_WHILE_ROAMING : 0;
        ...
        } else if (mSyncCount != null) {
            mAccount.setSyncLookback(Integer.parseInt(mSyncCount.getValue()));
        }
        ...
        if (info.syncContacts || info.syncCalendar || info.syncTasks || info.syncNotes) {
            ...
            ContentResolver.setSyncAutomatically(androidAcct, TaskContract.AUTHORITY,
                    mSyncTasks.isChecked());
            ContentResolver.setSyncAutomatically(androidAcct, NoteContract.AUTHORITY,
                    mSyncNotes.isChecked());
            }
