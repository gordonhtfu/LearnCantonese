...
import com.blackberry.email.account.AccountReconciler;
...
public class EmailBroadcastProcessorService extends IntentService {
    ...
    private static final String ACTION_DEVICE_POLICY_ADMIN = "com.blackberry.email.devicepolicy";
    ...
    /**
     * Entry point for {@link com.blackberry.email.account.SecurityPolicy.PolicyAdmin}.  These will
     * simply callback to {@link
     * com.blackberry.email.account.SecurityPolicy#onDeviceAdminReceiverMessage(Context, int)}.
     */
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (ACTION_BROADCAST.equals(action)) {
            ...
            if (Intent.ACTION_MY_PACKAGE_REPLACED.equals(broadcastAction)) {
                onAppUpgrade();
            } else if (Intent.ACTION_BOOT_COMPLETED.equals(broadcastAction)) {
                onBootCompleted();
            ...
            // TODO - Carl why did we need 2 broadcast receivers?  It appears that one of them
            // was turned off once the broadcast was received, I don't think that is necessary
            // and would propose that we just use a single receiver for all broadcasts.
            onAppUpgrade();
        }
    }
    ...
    private void fixPeriodicSyncs(final String amAccountType,
        ...
        for (android.accounts.Account amAccount : amAccounts) {
            ...
            removePeriodicSyncs(amAccount, MessageContract.AUTHORITY);
            ...
            if (syncInterval != null && syncInterval > 0) {
                ...
                ContentResolver.addPeriodicSync(amAccount, MessageContract.AUTHORITY, Bundle.EMPTY,
    ...
    private void onAppUpgrade() {
        ...
        // Now start any accounts that we are managing
        AccountUtils.syncAllEmailAccounts(this);
    }
