...
import com.blackberry.common.TempDirectory;
...
public abstract class EmailContent {
    ...
    public static synchronized String init(Context context) {
        if (AUTHORITY == null) {
           // final Resources res = context.getResources();
            //@NOTE CHANGING HOW THIS GETS SET
            EMAIL_PACKAGE_NAME = context.getPackageName();//res.getString(R.string.email_package_name);
            ...
            PROVIDER_PERMISSION = "com.blackberry.email.permission.ACCESS_EMAILSERVICES";//EMAIL_PACKAGE_NAME + ".permission.ACCESS_EMAILSERVICES";
            // Initialize subclasses
            Account.initAccount();
            Mailbox.initMailbox();
            VirtualMailbox.initVirtualMailbox();
    ...
    public interface MessageColumns {
        ...
        //Boolean, not included = 0, included = 1
        public static final String FLAG_INCLUDED = "flagIncluded";
    ...
    public static final class Message extends EmailContent implements SyncColumns, MessageColumns {
        ...
        public static final int CONTENT_FLAG_INCLUDED = 27;
        ...
        public static final int FLAG_TYPE_DRAFT = 1 << 25;
        ...
        public ContentValues toContentValues() {
            ...
            values.put(MessageColumns.FLAG_INCLUDED,mFlagIncluded);
            return values;
        }
        ...
        public void restore(Cursor cursor) {
            ...
            mFlagIncluded = cursor.getInt(CONTENT_FLAG_INCLUDED);
        }
         ...
         * (non-Javadoc)
         * @see com.blackberry.email.provider.EmailContent#save(android.content.Context)
         */
        @Override
        public Uri save(Context context) {
            ...
            try {
                ...
                if (doSave) {
                    ...
                    if (mAttachments != null) {
                        // Skip over items in the result array that are not attachments
                        int attachmentIndexOffset = 0;
                        String attachmentPath = Attachment.CONTENT_URI.getPathSegments().get(0);
                        while (attachmentIndexOffset < results.length) {
                            List<String> segments = results[attachmentIndexOffset].uri.getPathSegments();
                            if (segments.size() > 0 && segments.get(0).equals(attachmentPath)) {
                                break;
                            }
                        ...
                        for (int i = 0; i < mAttachments.size(); i++) {
                            ...
                            final int resultIndex = i + attachmentIndexOffset;
        ...
        // @TODO combine addSaveOps with this
        public ArrayList<Operation> getMsgOperations(Message msg, ArrayList<Operation> ops) {
            ArrayList<Operation> retAddedOps = new ArrayList<Operation>();

            boolean isNew = !msg.isSaved();
            ContentProviderOperation.Builder b;
            // First, save/update the message
            if (isNew) {
                b = ContentProviderOperation.newInsert(msg.mBaseUri);
            } else {
                b = ContentProviderOperation.newUpdate(msg.mBaseUri)
                        .withSelection(Message.RECORD_ID + "=?", new String[] {
            ...
            Operation msgOp = new Operation(b.withValues(msg.toContentValues()).build());
            ops.add(msgOp);
            // add to result container
            retAddedOps.add(msgOp);
            ...
            if (msg.mQuotedTextStartPos != 0) {
                cv.put(Body.QUOTED_TEXT_START_POS, msg.mQuotedTextStartPos);
            }
            // We'll need this if we're new
            int messageBackValue = ops.size() - 1;
            // Only create a body if we've got some data
            if (!cv.keySet().isEmpty()) {
                ...
                // Put our message id in the Body
                if (!isNew) {
                    cv.put(Body.MESSAGE_KEY, msg.mId);
                }
                b.withValues(cv);
                // If we're new, create a back value entry
                ...
                if (isNew) {
                    ...
                    bodyOps = new Operation(b, Body.MESSAGE_KEY, messageBackValue);
                } else {
            ...
            if (msg.mAttachments != null) {
                for (Attachment att : msg.mAttachments) {
                    ...
                    if (isNew) {
                        b.withValueBackReference(Attachment.MESSAGE_KEY, messageBackValue);
                        attOps = new Operation(b, Body.MESSAGE_KEY, messageBackValue);
                    } else {
                        attOps = new Operation(b.build());
                    }

                    ops.add(attOps);
                    retAddedOps.add(attOps);
                }
            }
    ...
    public interface AccountColumns {
        ...
        //THIS MAY BE TEMP ,WHILE SWITCHING EVERYTHING TO USE THE NEW ACCOUNT PROVIDER
        public static final String PIM_ACCOUNT_ID ="pimAccountId";
    ...
    public interface VirtualMailboxColumns {
        public static final String ID = "_id";
        // A foreign key to the Account that owns this mailbox
        public static final String ACCOUNT_KEY = "accountKey";
        // The type (role) of this mailbox
