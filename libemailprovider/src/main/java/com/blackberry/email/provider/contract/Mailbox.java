...
public class Mailbox extends EmailContent implements MailboxColumns, Parcelable {
    ...
    /** Selection for mailboxes that say they want to sync, plus outbox, for an account. */
    private static final String SYNC_ENABLED_MAILBOXES_BY_ACCOUNT = 
            MailboxColumns.SYNC_INTERVAL + "=1 and " + MailboxColumns.ACCOUNT_KEY + "=?";
    ...
    /** A generic type to indicate all Mail folders
     * NOTE: this type is just used internally and NOT saved to the database as a mailbox type.
     * It is simply used for internally processing to indicate that ALL mail folders should be
     * included in the action.
     */
    public static final int TYPE_ALL_MAIL = 11;

    /**
     * isMailFolder
     * @param mailboxType - type of mailbox to check
     * @return true if Mailbox is Inbox, Sent, Trash, or general Mail folder
     */
    public static boolean isMailFolder(int mailboxType) {
        switch(mailboxType) {
        case TYPE_INBOX:
        case TYPE_MAIL:
        case TYPE_SENT:
    ...
    public static final int TYPE_NOTES = 0x46;
    ...
    static {
        ...
        SYNCABLE_TYPES.put(TYPE_DRAFTS, false);
    ...
    public static ArrayList<Mailbox> restoreMailboxes(Context context, long[] ids) {
        ArrayList<Mailbox> mailboxes = new ArrayList<Mailbox>();
        if (ids != null && (ids.length > 0)) {
            for (long id: ids) {
                Mailbox mb = restoreMailboxWithId(context, id);
                if (mb != null) {
                    mailboxes.add(mb);
                }
            }
    ...
    public boolean isSyncable() {
        return (isSyncableType(mType) && mServerId != null && mServerId.length() > 0);
    }
    ...
    public Object[] getHashes() {
        ...
        hash[CONTENT_HIERARCHICAL_NAME_COLUMN]
                = mHierarchicalName;
        hash[CONTENT_LAST_FULL_SYNC_COLUMN] 
                = mLastFullSyncTime;
        return hash;
    }
    ...
    public String getTypeString() {
        ...
        case Mailbox.TYPE_CONTACTS:
            typeString = "Contacts";
            break;
        case Mailbox.TYPE_TASKS:
            typeString = "Tasks";
            break;
        case Mailbox.TYPE_NOTES:
            typeString = "Notes";
            break;
        default:
            typeString = "Unknown" + String.valueOf(mType);
            break;
        }
        return typeString;
    }
    ...
    public String toString() {
        ...
        if (mServerId != null) {
            return "[" + typeString + " " + mId + "," + mServerId + "]";
        } else {
            return "[" + typeString + " " + mId + "]";
        }
    }
    ...
    /**
     * Get the mailbox content for an account that should sync when we do a full account sync.
     * @param cr The {@link ContentResolver}.
     * @param accountId The id for the account that is pushing.
     * @return A cursor (with one column, containing ids) with all mailbox ids we should sync.
     */
    public static Cursor getMailboxesForSync(final ContentResolver cr, final long accountId) {
        ...
        return cr.query(Mailbox.CONTENT_URI, Mailbox.CONTENT_PROJECTION,
                SYNC_ENABLED_MAILBOXES_BY_ACCOUNT,
     ...
     * (excluding the Outbox - which is not a sync operation).
     * @param cr The {@link ContentResolver}.
     * @param accountId The id for the account that is pushing.
     * @return ArrayList<Long> - list of mailbox ids.
     */
    public static ArrayList<Long> getMailboxIdsForSync(final ContentResolver cr, final long accountId) {
        ArrayList<Long> mbIds = new ArrayList<Long>();
        Cursor c = getMailboxIdCursorForSync(cr, accountId);
        if (c != null) {
            try {
                while (c.moveToNext()) {
                    mbIds.add(c.getLong(0));
                }
            } finally {
                c.close();
            }
    ...
    /**
     * Get the cursor to mailbox IDs for an account that should sync when we do a full account sync
     * (excluding the Outbox which is not a sync operation).
     * @param cr The {@link ContentResolver}.
     * @param accountId The id for the account that is pushing.
     * @return A cursor (with one column, containing ids) with all mailbox ids we should sync.
     */
    public static Cursor getMailboxIdCursorForSync(final ContentResolver cr, final long accountId) {
        // We're sorting by mailbox type. The reason is that the inbox is type 0, other types
        // (e.g. Calendar and Contacts) are all higher numbers. Upon initial sync, we'd like to
        // sync the inbox first to improve perceived performance.
     ...
     */
    public static Cursor getMailboxIdCursorForSyncByType(final ContentResolver cr, final long accountId,
     ...
     * @return A cursor (with one column, containing ids) with all mailbox ids that match.
     */
    public static ArrayList<Long> getMailboxIdsForSyncByType(final ContentResolver cr, final long accountId,
            final int mailboxType) {
        ArrayList<Long> mbIds = new ArrayList<Long>();
        Cursor c = getMailboxIdCursorForSyncByType(cr, accountId, mailboxType);
        if (c != null) {
            try {
                while (c.moveToNext()) {
                    mbIds.add(c.getLong(0));
    ...
    /**
     * Get the mailbox content for an account that are configured for sync and have a specific type.
     * @param accountId The id for the account that is syncing.
     * @param mailboxType The type of the mailbox we're interested in.
     * @return A cursor (with one column, containing ids) with all mailbox ids that match.
     */
    public static Cursor getMailboxesForSyncByType(final ContentResolver cr, final long accountId,
            final int mailboxType) {
        return cr.query(Mailbox.CONTENT_URI, Mailbox.CONTENT_PROJECTION,
                SYNCING_AND_TYPE_FOR_ACCOUNT_SELECTION,
                new String[] { Integer.toString(mailboxType), Long.toString(accountId) }, null);
