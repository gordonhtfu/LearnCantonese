...
public class MessageStateChange extends MessageChangeLogTable {
    ...
    public MessageStateChange(final long messageKey,final String serverId, final long id,
    ...
    public static List<MessageStateChange> getChanges(final Context context, final long accountId,
        ...
        List<MessageStateChange> allChanges = getChanges(context, accountId, ignoreFavorites);
        if (allChanges == null) {
            return null;
        }

        LongSparseArray<List<MessageStateChange>> allChangesInMapForm = convertToChangesMap(allChanges);
    ...
    public static List<MessageStateChange> getChanges(final Context context, final long accountId,
        ...
        LongSparseArray<MessageStateChange> changesMap = getChangesWithCursor(ignoreFavorites, cr, c);

        // Prune no-ops.
        final int count = changesMap.size();
        final long[] unchangedMessages = new long[count];
        int unchangedMessagesCount = 0;
        final ArrayList<MessageStateChange> changes = new ArrayList<MessageStateChange>(count);
    ...
    private static LongSparseArray<MessageStateChange> getChangesWithCursor(final boolean ignoreFavorites, final ContentResolver cr, final Cursor c) {
        // Collapse rows acting on the same message.
        LongSparseArray<MessageStateChange> changesMap = new LongSparseArray<MessageStateChange>();
        try {
            while (c.moveToNext()) {
    ...
    public static LongSparseArray<List<MessageStateChange>> convertToChangesMap(
        ...
        final LongSparseArray<List<MessageStateChange>> changesMap = new LongSparseArray<List<MessageStateChange>>();
        for (final MessageStateChange change : changes) {
            List<MessageStateChange> list = changesMap.get(change.mMailboxId);
            if (list == null) {
                list = new ArrayList<MessageStateChange>();
