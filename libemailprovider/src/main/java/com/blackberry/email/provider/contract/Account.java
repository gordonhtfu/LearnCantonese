...
public class Account extends EmailContent implements AccountColumns, Parcelable {
    ...
    // Whether of not the account should download bodies while roaming
    public static final int FLAGS_DOWNLOAD_BODY_WHILE_ROAMING = 1<<14;
    ...
    public transient CookieStore mCookieStore;
    public transient long mCapabilities = 0;
    ...
    public static final int CONTENT_PIM_ACCOUNT_ID_COLUMN = 18;
    ...
    public void restore(Cursor cursor) {
        ...
        mPimAccountId = cursor.getLong(CONTENT_PIM_ACCOUNT_ID_COLUMN);
    }
     ...
     * TODO define sentinel values for "all", "1 month", etc.  See Account.java
     * @param value One of the {@link com.blackberry.email.service.SyncWindow} constants
     */
     ...
     * {@code content://com.blackberry.email.provider/account/UUID} format, which is safe to use
     * for desktop shortcuts.
     *
     * <p>We don't want to store _id in shortcuts, because
     * {@link com.blackberry.email.provider.AccountBackupRestore} won't preserve it.
     */
    ...
    /**
     * Parse {@link Uri} in the {@code content://com.blackberry.email.provider/account/ID} format
    ...
    public ContentValues toContentValues() {
        ...
        values.put(AccountColumns.PIM_ACCOUNT_ID, mPimAccountId);

        //how I hook into the pim id , yet all other email account flow is the same
        if(mId == NOT_SAVED){
            values.put(Account.RECORD_ID, mPimAccountId);
        }
    ...
    //Tacking on some helpers for our AccountProvider
}