...
import com.blackberry.common.perf.SimpleTimer;
...
import com.blackberry.email.ConversationInfo;
import com.blackberry.email.Folder;
import com.blackberry.email.FolderList;
import com.blackberry.email.MessageInfo;
import com.blackberry.email.NotificationController;
import com.blackberry.email.mail.Address;
import com.blackberry.email.preferences.MailPrefs;
import com.blackberry.email.preferences.Preferences;
import com.blackberry.email.provider.UIProvider.AccountCapabilities;
import com.blackberry.email.provider.UIProvider.AccountCursorExtraKeys;
import com.blackberry.email.provider.UIProvider.ConversationColumns;
import com.blackberry.email.provider.UIProvider.ConversationPriority;
import com.blackberry.email.provider.UIProvider.ConversationSendingState;
import com.blackberry.email.provider.UIProvider.DraftType;
...
import com.blackberry.email.service.EmailServiceStatus;
...
import com.blackberry.email.utils.MessageProviderUtilities;
import com.blackberry.email.utils.MessageConsumerTimeManager;
import com.blackberry.email.utils.MimeType;
import com.blackberry.email.utils.Utility;
import com.blackberry.ex.photo.provider.PhotoContract;
 ...
 * @author mblank
 */
public class EmailProvider extends PIMContentProviderBase {

    private static final String TAG = LogTag.getLogTag();

    private static final boolean ENABLE_MESSAGE_INTERCEPT_LOGIC = true;
     ...
     * register for this {@link android.content.Intent} and update accordingly.
     * However, this can be very broad and is NOT the preferred way of getting
     * notification.
     */
    private static final String ACTION_NOTIFY_MESSAGE_LIST_DATASET_CHANGED =
        "com.blackberry.email.MESSAGE_LIST_DATASET_CHANGED";
    ...
    private static final int VIRTUAL_MAILBOX_BASE = 0xB000;
    private static final int VIRTUAL_MAILBOX = VIRTUAL_MAILBOX_BASE;
    private static final int VIRTUAL_MAILBOX_ID = VIRTUAL_MAILBOX_BASE + 1;
    ...
    static {
        ...
        array.put(VIRTUAL_MAILBOX_BASE >> BASE_SHIFT, VirtualMailbox.TABLE_NAME);
        TABLE_NAMES = array;
    }
     ...
     * TODO: Do we actually need to synchronize across all DB access, not just
     * connection creation?
     */
    ...
    private static final HashMap<String, Integer> accountValues = new HashMap<String, Integer>() {
        /**
         * 
         */
    ...
    private static final HashMap<String, Integer> mailboxValues = new HashMap<String, Integer>() {
        {
            put("displayName", 1);
            put("serverId", 2);
            put("type", 3);
    ...
    private static final HashMap<String, Integer> messageValues = new HashMap<String, Integer>() {
        {
            put("subject", 1);
        }
    };
    ...
    protected static Uri INTEGRITY_CHECK_URI;
    public static Uri ACCOUNT_BACKUP_URI;
    private static Uri FOLDER_STATUS_URI;

    private SQLiteDatabase mDatabase;
    private SQLiteDatabase mBodyDatabase;
    private volatile DBHelper.DatabaseHelper mDbHelper;
    private volatile DBHelper.BodyDatabaseHelper mBodyDbHelper;

    private Handler mDelayedSyncHandler;
    private ArrayList<SyncRequestMessage> mDelayedSyncRequests = new ArrayList<SyncRequestMessage>();

    private static String EMAIL_MESSAGE_MIME_TYPE;
    private static String EMAIL_MESSAGES_MIME_TYPE;
    private static String EMAIL_ATTACHMENT_MIME_TYPE;
    private static String EMAIL_ATTACHMENTS_MIME_TYPE;
    private static String EMAIL_BODY_MIME_TYPE;
    private static String EMAIL_BODIES_MIME_TYPE;
    private static String EMAIL_MAILBOX_MIME_TYPE;
    private static String EMAIL_MAILBOXES_MIME_TYPE;
    private static String HOST_AUTH_MIME_TYPE;
    private static String HOST_AUTHS_MIME_TYPE;
    private static String ACCOUNT_MIME_TYPE;
    private static String ACCOUNTS_MIME_TYPE;

    protected void initMimetypes(Context context) {

        // /Message
        EMAIL_MESSAGE_MIME_TYPE = context.getString(R.string.message_mimetype);
        EMAIL_MESSAGES_MIME_TYPE = context.getString(R.string.messages_mimetype);
        // Attachment
        EMAIL_ATTACHMENT_MIME_TYPE = context.getString(R.string.attachment_mimetype);
        EMAIL_ATTACHMENTS_MIME_TYPE = context.getString(R.string.attachments_mimetype);
        // Body
        EMAIL_BODY_MIME_TYPE = context.getString(R.string.message_body_mimetype);
        EMAIL_BODIES_MIME_TYPE = context.getString(R.string.message_bodies_mimetype);
        // Mailbox
        EMAIL_MAILBOX_MIME_TYPE = context.getString(R.string.mailbox_mimetype);
        HOST_AUTHS_MIME_TYPE = context.getString(R.string.mailboxes_mimetype);
        // HostAuth
        HOST_AUTH_MIME_TYPE = context.getString(R.string.hostauth_mimetype);
        HOST_AUTHS_MIME_TYPE = context.getString(R.string.hostauths_mimetype);
        // Account
        ACCOUNT_MIME_TYPE = context.getString(R.string.account_mimetype);
        ACCOUNTS_MIME_TYPE = context.getString(R.string.accounts_mimetype);
    }
    ...
    /**
     * Orphan record deletion utility. Generates a sqlite statement like: delete
     * from
     * <table>
     * where <column> not in (select <foreignColumn> from <foreignTable>)
     * 
     ...
     * @param foreignColumn the column in the foreign table whose absence will
     *            trigger the deletion
     * @param foreignTable the foreign table
     */
    protected static void deleteUnlinked(SQLiteDatabase db, String table, String column,
    ...
    /**
     * Make sure that parentKeys match with parentServerId. When we sync
     * folders, we do two passes: First to create the mailbox rows, and second
     * to set the parentKeys. Two passes are needed because we won't know the
     * parent's Id until that row is inserted, and the order in which the rows
     * are given is arbitrary. If we crash while this operation is in progress,
     * the parent keys can be left uninitialized.
     * 
     * @param db
     */
    private void fixParentKeys(SQLiteDatabase db) {
        ...
        // Update the parentKey for each mailbox row to match the _id of the row
        // whose
        // serverId matches our parentServerId. This will leave parentKey blank
        // for any
        // row that does not have a parentServerId

        // This is kind of a confusing sql statement, so here's the actual text
        // of it,
        // for reference:
        //
        // update mailbox set parentKey = (select _id from mailbox as b where
        // mailbox.parentServerId=b.serverId and mailbox.parentServerId not null
        // and
        ...
        // Top level folders can still have uninitialized parent keys. Update
        // these
        // to indicate that the parent is -1.
        //
        // update mailbox set parentKey = -1 where parentKey=0 or parentKey is
        // null;
        db.execSQL("update " + Mailbox.TABLE_NAME + " set " + MailboxColumns.PARENT_KEY
    ...
    protected String getBodyDatabaseName() {
        return BODY_DATABASE_NAME;
    }
    ...
    protected void initializeDatabaseHelpers() {
        synchronized (sDatabaseLock) {
            ...
            mDbHelper = new DBHelper.DatabaseHelper(context, getDatabaseName());
            mBodyDbHelper = new DBHelper.BodyDatabaseHelper(context, getBodyDatabaseName());
        }
    }

    // Return all of the DB helpers associated with this provider.
    @Override
    protected SQLiteOpenHelper[] getDatabaseHelpers(boolean includeAttachedDbs) {
        if (includeAttachedDbs) {
            SQLiteOpenHelper helpers[] = new SQLiteOpenHelper[2];
            helpers[0] = mDbHelper;
            helpers[1] = mBodyDbHelper;
            return helpers;
        } else {
            SQLiteOpenHelper helpers[] = new SQLiteOpenHelper[1];
            helpers[0] = mDbHelper;
            return helpers;
        }
    }
    ...
    protected SQLiteDatabase getDatabase(Context context) {
        synchronized (sDatabaseLock) {
            ...
            mBodyDatabase = mBodyDbHelper.getWritableDatabase();
            ...
            // Delete orphaned mailboxes/messages/policies (account no longer
            // exists)
    ...
    private static void restoreIfNeeded(Context context, SQLiteDatabase mainDatabase) {
        ...
        // If there's a legacy backup, create a new-style backup and delete the
        // legacy backup
        // In the 1:1000000000 chance that the user gets an app update just as
        // his database becomes
        // corrupt, oh well...
    ...
    protected static void deleteMessageOrphans(SQLiteDatabase database, String tableName) {
        if (database != null) {
        ...
        try {
            ...
            switch (match) {
                ...
                case MESSAGE_SELECTION:
                    Cursor findCursor = db.query(tableName, Message.ID_COLUMN_PROJECTION,
                            selection,
                    ...
                    // These are cases in which one or more Messages might get
                    // deleted, either by
                ...
                case MESSAGE_ID:
                    // Handle lost Body records here, since this cannot be done
                    // in a trigger
                    // The process is:
                    ...
                    //
                    // Bodies are auto-deleted here; Attachments are
                    // auto-deleted via trigger
            ...
            switch (match) {
                ...
                case QUICK_RESPONSE_ID:
                    ...
                    if (match == SYNCED_MESSAGE_ID) {
                        // For synced messages, first copy the old message to
                        // the deleted table and
                        // delete it from the updated table (in case it was
                        // updated first)
                        // Note that this is all within a transaction, for
                        // atomicity
                    ...
                    if (match == ACCOUNT_ID) {
                        ...
                        notifyChange(UIPROVIDER_ALL_ACCOUNTS_NOTIFIER, null);
            ...
            if (messageDeletion) {
                if (match == MESSAGE_ID) {
                    //I know the Msg is deletes so will now sync against domain provider
                    MessageProviderUtilities.deleteMessage(context, id);
    ...
    public String getType(Uri uri) {
        ...
        switch (match) {
            case BODY_ID:
                return EMAIL_BODY_MIME_TYPE;
            case BODY:
                return EMAIL_BODIES_MIME_TYPE;
            case UPDATED_MESSAGE_ID:
            case MESSAGE_ID:
                // NOTE: According to the framework folks, we're supposed to
                // invent mime types as
                // a way of passing information to drag & drop recipients.
                // If there's a mailboxId parameter in the url, we respond with
                // a mime type that
                // has -n appended, where n is the mailboxId of the message. The
                // drag & drop code
                // uses this information to know not to allow dragging the item
                // to its own mailbox
            ...
            case UPDATED_MESSAGE:
            case MESSAGE:
                return EMAIL_MESSAGES_MIME_TYPE;
            case MAILBOX:
                return EMAIL_MAILBOXES_MIME_TYPE;
            case MAILBOX_ID:
                return EMAIL_MAILBOX_MIME_TYPE;
            case ACCOUNT:
            ...
            case ATTACHMENTS_MESSAGE_ID:
            case ATTACHMENT:
                return EMAIL_ATTACHMENTS_MIME_TYPE;
            case ATTACHMENT_ID:
                return EMAIL_ATTACHMENT_MIME_TYPE;
            case HOSTAUTH:
                return HOST_AUTHS_MIME_TYPE;
            case HOSTAUTH_ID:
                return HOST_AUTH_MIME_TYPE;
            default:
                return null;
        }
    }

    // These URIs are used for specific UI notifications. We don't use
    // EmailContent.CONTENT_URI
    // as the base because that gets spammed.
    // These can't be statically initialized because they depend on
    ...
    protected boolean isValidInsertion(int uriMatch, ContentValues values) {

        boolean result;

        switch (uriMatch) {
            case MESSAGE:
                result = values.getAsLong(Message.MAILBOX_KEY) != null
                        && values.getAsLong(Message.ACCOUNT_KEY) != null;
                break;
            default:
                result = true;
        }
    ...
    public Uri pimInsert(Uri uri, ContentValues values) {
        Log.d(TAG, "EmailProvider::insert: " + uri);
        Set<Entry<String, Object>> valueSet = null;
        if (values != null) {
            valueSet = values.valueSet();
        }
        int match = findMatch(uri, "insert");

        if (!isValidInsertion(match, values)) {
            return null;
        }
        ...
        // Just so I don't make a mess of the switch statement, I will
        // experiment with the
        // Message Consumers here
        // Got to watch it here as both incoming and outgoing messages will come
        // through this pass
        // Need to know - for now I will use the serverSyncId and maybe the
        // Mailbox type
        boolean invokeIncomingInterceptLogic = false;
        if (ENABLE_MESSAGE_INTERCEPT_LOGIC) {
            if (match == MESSAGE && values.getAsString(EmailContent.SyncColumns.SERVER_ID) != null) {
                if (hasRegisteredInterceptReceivers(context,
                        EMAIL_INCOMING_MESSAGE_INTERCEPT_INTENT_ACTION)) {
                    invokeIncomingInterceptLogic = true;
                    // flip flag here to be excluded
                    values.put(MessageColumns.FLAG_INCLUDED, 0);
                }
            }
        }
        ...
        try {
            switch (match) {
                case MESSAGE: {
                    resultUri = handleMessageInsert(context, uri, values,
                            invokeIncomingInterceptLogic);
                    break;
                }
                case MAILBOX: {
                    resultUri = handleMailboxInsert(context, uri, values);
                    break;
                }
                case BODY: {
                    resultUri = handleBodyInsert(context, uri, values);
                    break;
                }
                case ATTACHMENT: {
                    resultUri = handleAttachmentInsert(context, uri, values);
                    break;
                }
                case ACCOUNT:
                case HOSTAUTH:
     ...
     * @param uri
     * @param values
     * @param invokeIncomingInterceptLogic
     * @return
     */
    protected Uri handleMessageInsert(Context context,
            Uri uri,
            ContentValues values,
            boolean invokeIncomingInterceptLogic) {

        long longId = this.getDatabase(context).insert(Message.TABLE_NAME, "foo", values);
        Uri resultUri = ContentUris.withAppendedId(uri, longId);

        // NOTE THIS WAS A PROTOTYPE FINAL CODE MAY ONLY LIVE AT DOMAIN LAYER
        // based on above logic
        if (invokeIncomingInterceptLogic) {
            sendIncomingInterceptBroadcastIntent(resultUri, values);
            return resultUri;
        }

        MessageProviderUtilities.insertMessage(context, resultUri, values, getType(resultUri));
     ...
     * @return
     */
    protected Uri handleMailboxInsert(Context context, Uri uri, ContentValues values) {

        long longId = this.getDatabase(context).insert(Mailbox.TABLE_NAME, "foo", values);
        Uri resultUri = ContentUris.withAppendedId(uri, longId);

        if (values.containsKey(MailboxColumns.TYPE)) {
            if (values.getAsInteger(MailboxColumns.TYPE) < Mailbox.TYPE_NOT_EMAIL) {

                MessageProviderUtilities.insertUpdateFolder(context, resultUri, values, true);
    ...
    protected void handleMailboxUpdate(Context context, Uri uri, ContentValues values) {
        MessageProviderUtilities.insertUpdateFolder(context, uri, values, false);
    }
     ...
     * @return
     */
    protected Uri handleBodyInsert(Context context, Uri uri, ContentValues values) {
        long longId = this.getDatabase(context).insert(Body.TABLE_NAME, "foo", values);
        Uri resultUri = ContentUris.withAppendedId(uri, longId);

        MessageProviderUtilities.insertBody(context, resultUri, values);
        return resultUri;
    }
     ...
     * @return
     */
    protected Uri handleAttachmentInsert(Context context, Uri uri, ContentValues values) {

        long longId = this.getDatabase(context).insert(Attachment.TABLE_NAME, "foo", values);
        Uri resultUri = ContentUris.withAppendedId(uri, longId);
        ...
        MessageProviderUtilities.insertAttachment(context, resultUri, values);

        return resultUri;
    }

    public static final String INCOMING_PERMISSIONS = "com.blackberry.permission.INCOMING_INTERCEPTOR";
    public static final String OUTGOING_PERMISSIONS = "com.blackberry.permission.OUTGOING_INTERCEPTOR";
    public static final String EMAIL_INCOMING_MESSAGE_INTERCEPT_INTENT_ACTION = "com.blackberry.email.message.incoming.INTERCEPT";
    public static final String EMAIL_OUTGOING_MESSAGE_INTERCEPT_INTENT_ACTION = "com.blackberry.email.message.outgoing.INTERCEPT";
    public static final String EXTRA_INTERCEPTED = "com.blackberry.extra.intercepted";
    public static final String EXTRA_MESSAGE_URI = "com.blackberry.extra.message.uri";
    public static final String EXTRA_BODY_URI = "com.blackberry.extra.body.uri";
    public static final String EXTRA_ATTACHMENTS_URI = "com.blackberry.extra.attachments.uri";
    public static final String EXTRA_EMAIL_MESSAGE_VALUES = "com.blackberry.extra.message.values";
    public static final String EXTRA_MAILBOX_ID = "com.blackberry.extra.mailbox.id";
    public static final String EXTRA_EXTRA = "com.blackberry.extra.extra";
    // for measuring performance
    public static final String EXTRA_CREATE_TIME_PERF = "com.blackberry.extra.createtime.perf";

    private void sendIncomingInterceptBroadcastIntent(Uri messageUri, ContentValues values) {

        Intent messageInterceptIntent = new Intent(EMAIL_INCOMING_MESSAGE_INTERCEPT_INTENT_ACTION);

        // adding some payload data, this may change going forward
        long messageId = Long.parseLong(messageUri.getPathSegments().get(1));
        long mailboxId = values.getAsLong(Message.MAILBOX_KEY) != null ? values.getAsLong(
                Message.MAILBOX_KEY).longValue() : -1;

        messageInterceptIntent.putExtra(EXTRA_MESSAGE_URI, messageUri.toString());
        messageInterceptIntent.putExtra(EXTRA_BODY_URI, EmailContent.Body.CONTENT_URI.toString());
        messageInterceptIntent.putExtra(EXTRA_EMAIL_MESSAGE_VALUES, values);

        if (mailboxId != -1) {
            messageInterceptIntent.putExtra(EXTRA_MAILBOX_ID, mailboxId);
        }

        Boolean hasAttachments = values.getAsBoolean(Message.FLAG_ATTACHMENT);
        if (hasAttachments != null && hasAttachments.booleanValue()) {
            messageInterceptIntent.putExtra(EXTRA_ATTACHMENTS_URI,
                    ContentUris.withAppendedId(Attachment.MESSAGE_ID_URI, messageId).toString());
        }

        // for measuring metrics I am adding time to the Intent
        messageInterceptIntent.putExtra(EXTRA_CREATE_TIME_PERF, SystemClock.uptimeMillis());

        this.getContext().sendOrderedBroadcast(messageInterceptIntent, INCOMING_PERMISSIONS,
                new BroadcastReceiver() {

            public void onReceive(Context context, Intent intent) {
               // Timer.startTiming("EMAILSERVICE INTERCEPT ONRECEIVE");
                ...
                timer.mark("EMAILSERVICE INTERCEPT ONRECEIVE after getting return data");
                        if (results != null && extras != null) {
                            boolean messageIntercepted = results.getBoolean(EXTRA_INTERCEPTED,
                                    false);
                            LogUtils.w(TAG, "sendInterceptBroadcastIntent INTERCEPTED="
                                    + messageIntercepted);
                            // if intercepted, fire delete
                            // if not then fire update or just let timer task
                            // fire
                    String strUri = extras.getString(EXTRA_MESSAGE_URI);
                            if (strUri != null) {
                                if (messageIntercepted) {
                                    context.getContentResolver().delete(Uri.parse(strUri), null,
                                            null);

                            timer.mark("EMAILSERVICE INTERCEPT ONRECEIVE after delete return data");
                                    LogUtils.w(TAG,
                                            "sendInterceptBroadcastIntent WAS intercepted and removed");
                                } else {

                                    // @TODO update message flag state to normal
                                    // and cancel timerTask
                            ContentValues updatedValues = new ContentValues();
                            updatedValues.put(MessageColumns.FLAG_INCLUDED, 1);
                                    context.getContentResolver().update(Uri.parse(strUri),
                                            updatedValues, null, null);

                            timer.mark("EMAILSERVICE INTERCEPT ONRECEIVE after update return data");
                                    LogUtils.w(TAG,
                                            "sendInterceptBroadcastIntent WAS NOT intercepted and removed");
                        }
                    }

                            // will log total time of processing the BroadCast
                            // Intent
                            long intentCreateTime = extras.getLong(EXTRA_CREATE_TIME_PERF, 0);
                            long now = SystemClock.uptimeMillis();
                            LogUtils.i("SimpleTimer", "[%s] %sms elapsed",
                                    "sendInterceptBroadcastIntent onReceive Done",
                           now - intentCreateTime);
                }
                LogUtils.w(TAG, "sendInterceptBroadcastIntent onReceive Done");

               // Timer.stopTiming("EMAILSERVICE INTERCEPT ONRECEIVE");
                    }
                }, null, Activity.RESULT_OK, null, null);

        // will activate timeout here
        MessageConsumerTimeManager.getInstance()
        .addIncomingTimeoutMessageTask(messageUri, this.getContext());

    }

    private void sendOutgoingInterceptBroadcastIntent(Message msg, Mailbox mailbox, Bundle extras) {

        Intent messageInterceptIntent = new Intent(EMAIL_OUTGOING_MESSAGE_INTERCEPT_INTENT_ACTION);

        // adding Extra data, Note may need to do a real query to fully get all
        // data memebers
        // For now will just include the uri, will add full message data in
        // future
        // @TODO finalize the data that we will be sending out.
        messageInterceptIntent.putExtra(EXTRA_EMAIL_MESSAGE_VALUES, msg.toContentValues());
        messageInterceptIntent.putExtra(EXTRA_MAILBOX_ID, mailbox.mId);// may be
                                                                       // redundant
                                                                       // bases
                                                                       // on msg
                                                                       // values
        messageInterceptIntent.putExtra(EXTRA_MESSAGE_URI, msg.getUri().toString());
        messageInterceptIntent.putExtra(EXTRA_BODY_URI, EmailContent.Body.CONTENT_URI.toString());

        if (msg.mFlagAttachment) {
            messageInterceptIntent.putExtra(EXTRA_ATTACHMENTS_URI,
                    ContentUris.withAppendedId(Attachment.MESSAGE_ID_URI, msg.mId).toString());
        }
        // ran into issue with attachments and opened fds being sent in
        // broadcast
        // extras.remove("opened_fds");
        // messageInterceptIntent.putExtra(EXTRA_EXTRA, extras);

        // for measuring metrics I am adding time to the Intent
        messageInterceptIntent.putExtra(EXTRA_CREATE_TIME_PERF, SystemClock.uptimeMillis());

        this.getContext().sendOrderedBroadcast(messageInterceptIntent, OUTGOING_PERMISSIONS,
                new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                timer.mark("EMAILSERVICE OUTGOING INTERCEPT ONRECEIVE after getting return data");
                        if (results != null && extras != null) {
                            boolean messageIntercepted = results.getBoolean(EXTRA_INTERCEPTED,
                                    false);
                            LogUtils.w(TAG, "sendOutgoingInterceptBroadcastIntent INTERCEPTED="
                                    + messageIntercepted);

                            ContentValues msgValues = (ContentValues) extras
                                    .get(EXTRA_EMAIL_MESSAGE_VALUES);

                            long mailboxId = extras.getLong(EXTRA_MAILBOX_ID, -1);
                            if (msgValues != null && mailboxId != -1) {
                                if (messageIntercepted) {
                                    long accountId = msgValues
                                            .getAsLong(MessageColumns.ACCOUNT_KEY);
                            final Mailbox mailbox =
                                            getMailboxByAccountIdAndType(accountId,
                                                    Mailbox.TYPE_DRAFTS);
                            ContentValues updatedValues = new ContentValues();
                            updatedValues.put(MessageColumns.MAILBOX_KEY, mailbox.mId);
                            updatedValues.put(MessageColumns.FLAG_INCLUDED, 1);
                            ...
                            NotificationController.getInstance(context).
                                            showOutgoingMessageBlockedNotification(accountId,
                                                        msgValues.getAsString(MessageColumns.SUBJECT));

                            timer.mark("EMAILSERVICE OUTGOING INTERCEPT ONRECEIVE after delete return data");
                                    LogUtils.w(TAG,
                                            "sendOutgoingInterceptBroadcastIntent WAS intercepted and removed");
                                } else {
                                    // so flip flag back to included and then do
                                    // a sync
                            String strUri = extras.getString(EXTRA_MESSAGE_URI);
                            ...
                            updatedValues.put(MessageColumns.FLAG_INCLUDED, 1);
                                    ...
                                    updatedValues, null, null);
                                    // sync it now
                                    startSync(Mailbox.restoreMailboxWithId(context, mailboxId), 0);
                            timer.mark("EMAILSERVICE INTERCEPT ONRECEIVE after update return data");
                                    LogUtils.w(TAG,
                                            "sendOutgoingInterceptBroadcastIntent WAS NOT intercepted and removed");
                        }
                            } else {
                                LogUtils.w(TAG,
                                        "sendOutgoingInterceptBroadcastIntent ERROR Missing Data.");
                            ...
                            long now = SystemClock.uptimeMillis();
                            LogUtils.i("SimpleTimer", "[%s] %sms elapsed",
                                    "sendOutgoingInterceptBroadcastIntent onReceive Done",
                           now - intentCreateTime);
                        } else {
                            LogUtils.w(TAG,
                                    "sendOutgoingInterceptBroadcastIntent ERROR Missing Data.");
                ...
                LogUtils.w(TAG, "sendOutgoingInterceptBroadcastIntent onReceive Done");
    }
                }, null, Activity.RESULT_OK, null, null);
    }

    public boolean hasRegisteredInterceptReceivers(Context context, String intentType) {
        // @TODO add to a map so query is in executed everytime - can also add a
        // receiver for listening
        // to install/uninstall events so that we then invoke a requery.
        Timer.startTiming("EMAILSERVICE CHECK INTERCEPT RECEIVERS");
        Intent interceptIntent = new Intent(intentType);
        List<ResolveInfo> results =
                context.getPackageManager().queryBroadcastReceivers(interceptIntent,
                        PackageManager.GET_RESOLVED_FILTER | PackageManager.GET_META_DATA);

        boolean bRet = false;
        if (results.size() > 0) {
            bRet = true;
            // Doing test to see if I can get metadata about the receiver
            // This could allow us in future to put some extra criteria logic
            // into his flow, as that we try
            // to only effect some of the Messages and not all of them, for
            // example Sender address.
            for (ResolveInfo info : results) {

                if (info.activityInfo != null) {
                    Bundle receiverMetaData = info.activityInfo.metaData;
                    if (receiverMetaData != null) {
                        // get our defined metadata
                       // test.metadata.future
                        String value = receiverMetaData.getString(" test.metadata.future");
                       LogUtils.d(TAG, "recevier METADATA value = " + value);
        ...
        Timer.stopTiming("EMAILSERVICE CHECK INTERCEPT RECEIVERS");
        return bRet;
    }
    ...
    private static void init(final Context context) {
        ...
        synchronized (sURIMatcher) {
            ...
            // Not currently used
            /*
             * UIPROVIDER_SETTINGS_NOTIFIER = Uri.parse("content://" +
             * uiNotificationAuthority + "/uisettings");
             */
            ...
            // Special URI to reset the new message count. Only update works,
            // and values
            // will be ignored.
            ...
            // ** NOTE For now, the accountKey must be set manually in the
            // values!
            ...
            // Virtual mailboxes
            sURIMatcher.addURI(EmailContent.AUTHORITY, "virtualMailbox", VIRTUAL_MAILBOX);
            sURIMatcher.addURI(EmailContent.AUTHORITY, "virtualMailbox/*", VIRTUAL_MAILBOX_ID);

            // All messages
            sURIMatcher.addURI(EmailContent.AUTHORITY, "message", MESSAGE);
            // A specific message
            // insert into this URI causes an attachment to be added to the
            // message
            ...
            // The attachments of a specific message (query only) (insert &
            // delete TBD)
            ...
            /**
             * THE URIs BELOW THIS POINT ARE INTENDED TO BE USED BY SYNC
             * ADAPTERS ONLY THEY REFER TO DATA CREATED AND MAINTAINED BY CALLS
             * TO THE SYNCED_MESSAGE_ID URI BY THE UI APPLICATION
             */
            ...
            // We listen to everything trailing uifolder/ since there might be
            // an appVersion
    ...
    public void checkDatabases() {
        synchronized (sDatabaseLock) {
            ...
            // Look for orphans, and delete as necessary; these must always be
            // in sync
    ...
    public Cursor pimQuery(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        // LogUtils.w(TAG, "query() uri =" +uri.toString());
        Cursor c = null;
        int match;
        try {
        ...
        try {
            switch (match) {
                ...
                case UI_ALL_FOLDERS:
                    // For now, we don't allow selection criteria within these
                    // queries
                ...
                case UI_FOLDER_LOAD_MORE:
                    c = uiFolderLoadMore(getMailbox(uri), false);
                    return c;
                ...
                case BODY:
                    ...
                    fetchMessageBodyIfNeeded(c, uri, projection);
                    break;
                case MESSAGE:
                    // put HACK in for check for outgoing messages and ensuring
                    // they are flagged as included
                    if (selection != null && selection.contains("mailBoxKey")) {
                        selection += " AND flagIncluded = 1";
                    }
                case UPDATED_MESSAGE:
                case DELETED_MESSAGE:
                case ATTACHMENT:
                case MAILBOX:
                case VIRTUAL_MAILBOX:
                case ACCOUNT:
                case HOSTAUTH:
                case POLICY:

                    // so for outgoing blocked messages I need to see if the
                    // query wants them
                    // and if I have blocked items they are excluded
                    // mailBoxKey=<outgoing id> AND flagIncluded=1;
                    // may need to know what clients are requesting the data as
                    // well **MFL
                    c = db.query(tableName, projection,
                            selection, selectionArgs, null, null, sortOrder, limit);
            ...
            checkDatabases();
            e.printStackTrace();
            // will through this as to not let user know it was a db thing
            throw new IllegalArgumentException("Unknown URI " + uri);
        } finally {
            if (c == null) {
                // This should never happen, but let's be sure to log it...
                // TODO: There are actually cases where c == null is expected,
                // for example
                // UI_FOLDER_LOAD_MORE.
    ...
    private static int copyAccountTables(SQLiteDatabase fromDatabase, SQLiteDatabase toDatabase) {
        ...
        // Lock both databases; for the "from" database, we don't want anyone
        // changing it from
        ...
        try {
            ...
            try {
                ...
                try {
                    // Loop through accounts, copying them and associated host
                    // auth's
                    while (c.moveToNext()) {
                        Account account = new Account();
                        account.restore(c);

                        // Clear security sync key and sync key, as these were
                        // specific to the
                        // state of the account, and we've reset that...
                        // Clear policy key so that we can re-establish policies
                        // from the server
                        // TODO This is pretty EAS specific, but there's a lot
                        // of that around
                        ...
                        // The account might have gone away, though very
                        // unlikely
                        if (hostAuth == null)
                            continue;
                        ...
                        if (account.mHostAuthKeySend > 0) {
                            ...
                            // Belt and suspenders; I can't imagine that this is
                            // possible,
                            // since we checked the validity of the account
                            // above, and the
    ...
    /**
     * Formatting string to generate the SQL statement for inserting into
     * MessageMove. The formatting parameters are: table name, message id x 4,
     * destination folder id, message id, destination folder id. Duplications
     * are needed for sub-selects.
     */
    private static final String MESSAGE_MOVE_INSERT = MESSAGE_CHANGE_LOG_TABLE_INSERT_PREFIX
            ...
            + "%d,"
            + "(select " + Mailbox.SERVER_ID + " from " + Mailbox.TABLE_NAME
            + " where _id=(select "
    ...
    /**
     * Formatting string to generate the SQL statement for inserting into
     * MessageStateChange. The formatting parameters are: table name, message id
     * x 4, new flag read, message id, new flag favorite. Duplications are
     * needed for sub-selects.
     */
    private static final String MESSAGE_STATE_CHANGE_INSERT = MESSAGE_CHANGE_LOG_TABLE_INSERT_PREFIX
            + MessageStateChange.OLD_FLAG_READ
            + ","
            + MessageStateChange.NEW_FLAG_READ
            + ","
            + MessageStateChange.OLD_FLAG_FAVORITE
            + ","
            + MessageStateChange.NEW_FLAG_FAVORITE
            + MESSAGE_CHANGE_LOG_TABLE_VALUES_PREFIX
            + "(select "
            + Message.FLAG_READ
            + " from "
    ...
    // select count(*) from (select count(*) as dupes from Mailbox where
    // accountKey=?
    ...
    private static final String ACCOUNT_INTEGRITY_SQL =
            "select count(*) from (select count(*) as dupes from " + Mailbox.TABLE_NAME +
                    " where accountKey=? group by " + MailboxColumns.SERVER_ID
                    + ") where dupes > 1";

    // Query to get the protocol for a message. Temporary to switch between new
    // and old upsync
    ...
    /**
     * Query to get the protocol and email address for an account. Note that
     * this uses {@link #INDEX_PROTOCOL} and {@link #INDEX_EMAIL_ADDRESS} for
     * its columns.
     */
    ...
    private static boolean restartPush(final Context context, final SQLiteDatabase db,
            final String id) {
        final Cursor c = db.rawQuery(GET_ACCOUNT_DETAILS, new String[] {
            id
        });
        if (c != null) {
            try {
                if (c.moveToFirst()) {
                    ...
                    // Only restart push for EAS accounts that have completed
                    // initial sync.
     ...
     * @param context A {@link Context}.
     * @param db The {@link SQLiteDatabase}.
     * @param values The {@link ContentValues} that were updated for the
     *            mailbox.
    ...
    public int pimUpdate(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (uri.equals(INTEGRITY_CHECK_URI)) {
            checkDatabases();
            return 0;
        } else if (uri.equals(ACCOUNT_BACKUP_URI)) {
        ...
        try {
            switch (match) {
                ...
                case ACCOUNT_CHECK:
                    ...
                    Cursor ic = null;
                    try {
                        ic = db.rawQuery(ACCOUNT_INTEGRITY_SQL, new String[] {
                            id
                        });
                        if (ic.moveToFirst()) {
                            res = ic.getInt(0);
                ...
                case POLICY_ID:
                    id = uri.getPathSegments().get(1);
                    if (match == SYNCED_MESSAGE_ID) {
                        // TODO: Migrate IMAP to use
                        // MessageMove/MessageStateChange as well.
                        boolean isEas = false;
                        long mailboxId = -1;
                        long accountId = -1;
                        ...
                        if (isEas) {
                            ...
                            Integer flagFavorite = values
                                    .getAsInteger(MessageColumns.FLAG_FAVORITE);
                            ...
                            // Request a sync for the messages mailbox so the
                            // update will upsync.
                            // This is normally done with
                            // ContentResolver.notifyUpdate() but doesn't
                            // work for Exchange because the Sync Adapter is
                            // declared as
                            // android:supportsUploading="false". Changing it to
                            // true is not trivial
                            // because that would require us to protect all
                            // calls to notifyUpdate()
                            // with syncToServer=false except in cases where we
                            // actually want to
                            // upsync.
                            // TODO: Look into making Exchange Sync Adapter
                            // supportsUploading=true
                            // Since we can't use the Sync Manager
                            // "delayed-sync" feature which
                            // applies only to UPLOAD syncs, we need to do this
                            // ourselves. The
                            // purpose of this is not to spam syncs when making
                            // frequent
                            // modifications.
                            ...
                            if (amAccount != null) {
                                ...
                                synchronized (mDelayedSyncRequests) {
                                    int index = mDelayedSyncRequests.indexOf(request);
                                    if (index == -1) {
                                        ...
                                        SyncRequestMessage oldRequest = mDelayedSyncRequests
                                                .get(index);
                                        oldRequest.mNumChanges++;
                                    }
                                }
                            } else {
                                LogUtils.d(TAG,
                            ...
                            // For synced messages, first copy the old message
                            // to the updated table
                            // Note the insert or ignore semantics, guaranteeing
                            // that only the first
                            // update will be reflected in the updated message
                    ...
                    result = db.update(tableName, values, whereWithId(id, selection),
                            selectionArgs);
                    if (match == MESSAGE_ID || match == SYNCED_MESSAGE_ID) {
                        //note both backend service and device msg updates hit this path
                        handleMessageUpdated(context,uri,id,values);
                    } else if (match == ATTACHMENT_ID) {
                        ...
                        // Notify UI if necessary; there are only two columns we
                        // can change that
                        ...
                        handleMailboxUpdate(context, uri, values);
                    ...
                    } else if (match == BODY_ID) {
                        if ((values.containsKey(BodyColumns.HTML_CONTENT) || values
                                .containsKey(BodyColumns.TEXT_CONTENT))
                                && values.containsKey(BodyColumns.MESSAGE_KEY)) {
                            notifyUI(UIPROVIDER_MESSAGE_NOTIFIER,
                                    values.getAsLong(BodyColumns.MESSAGE_KEY));
                        }
                    }
                    break;
                case BODY:
                    ...
                    if (result == 0 && selection.equals(Body.SELECTION_BY_MESSAGE_KEY)) {
                        // TODO: This is a hack. Notably, the selection equality
                        // test above
                        // is hokey at best.
                ...
                case ACCOUNT_RESET_NEW_COUNT_ID:
                    ...
                    notificationUri = Account.CONTENT_URI; // Only notify
                                                           // account cursors.
                    break;
    ...
    @Override
    public Bundle pimCall(String method, String arg, Bundle extras) {
        LogUtils.d(TAG, "EmailProvider#call(%s, %s)", method, arg);
        // Handle queries for the device friendly name.
        // TODO: This should eventually be a device property, not defined by the
        // app.
        if (TextUtils.equals(method, EmailContent.DEVICE_FRIENDLY_NAME)) {
            final Bundle bundle = new Bundle(1);
            // TODO: For now, just use the model name since we don't yet have a
            // user-supplied name.
        ...
        if (TextUtils.equals(method, Mailbox.FIX_PARENT_KEYS_METHOD)) {
        ...
        if (TextUtils.equals(method, UIProvider.AccountCallMethods.SEND_MESSAGE) ||
                TextUtils.equals(method, UIProvider.AccountCallMethods.SAVE_MESSAGE) ||
                TextUtils.equals(method, UIProvider.AccountCallMethods.SET_CURRENT_ACCOUNT) ||
                TextUtils.equals(method, UIProvider.AccountCallMethods.REMOTE_SEARCH) ||
                TextUtils.equals(method, UIProvider.AccountCallMethods.FETCH_MORE)) {
            // Handle send & save.
            ...
            } else if (TextUtils.equals(method, UIProvider.AccountCallMethods.REMOTE_SEARCH)) {
                String filter = extras.getString(UIProvider.SearchQueryParameters.QUERY);
                int limit = extras.getInt("_limit", -1);
                folderUri = apiRemoteSearch(accountId, filter, limit);
            } else if (TextUtils.equals(method, UIProvider.AccountCallMethods.FETCH_MORE)) {
                folderUri = extras.getString(UIProvider.FolderColumns.URI);
                Mailbox searchMailbox = getMailbox(Uri.parse(folderUri));
                if(searchMailbox != null) {
                    uiFolderLoadMore(searchMailbox, true);
                }
            } else {
            ...
            if (messageUri != null) {
                ...
                result = new Bundle(1);
                result.putString(UIProvider.FolderColumns.URI, folderUri);
            } else {
                result = null;
            }
    ...
    public ContentProviderResult[] applyBatch(ArrayList<ContentProviderOperation> operations)
        ...
        //need to do this now as some logic has been moved to base class
        this.getDatabase(this.getContext());
        ...
        switch (match) {
            case ATTACHMENTS_CACHED_FILE_ACCESS:
                ...
                if (cachedFilePath != null) {
                    ...
                    // check our permissions rather than the permissions of
                    // whatever
                    // code is calling us.
 ...
 // exposed for testing
    ...
    private Cursor getMailboxMessageCount(Uri uri) {
        SQLiteDatabase db = getDatabase(getContext());
        String mailboxId = uri.getLastPathSegment();
        return db.rawQuery("select count(*) from Message where mailboxKey=? AND flagIncluded = 1 ",
                new String[] {
                    mailboxId
                });
    }
        ...
        if (sMessageListMap == null) {
            sMessageListMap = ProjectionMap
                ...
                .add(UIProvider.ConversationColumns.DATE_RECEIVED_MS, MessageColumns.TIMESTAMP)
                    .add(UIProvider.ConversationColumns.HAS_ATTACHMENTS,
    ...
    /**
     * Generate UIProvider draft type; note the test for "reply all" must come
     * before "reply"
     */
    ...
    /**
     * Mapping of UIProvider columns to EmailProvider columns for a detailed
     * message view in UnifiedEmail
     */
    private static ProjectionMap getMessageViewMap() {
        if (sMessageViewMap == null) {
            sMessageViewMap = ProjectionMap
                    .builder()
                ...
                .add(UIProvider.MessageColumns.SERVER_ID, SyncColumns.SERVER_ID)
                    .add(UIProvider.MessageColumns.URI,
                ...
                .add(UIProvider.MessageColumns.BCC, EmailContent.MessageColumns.BCC_LIST)
                    .add(UIProvider.MessageColumns.REPLY_TO,
                            EmailContent.MessageColumns.REPLY_TO_LIST)
                ...
                .add(UIProvider.MessageColumns.MESSAGE_ACCOUNT_URI,
                        uriWithColumn("uiaccount", MessageColumns.ACCOUNT_KEY))
                    .add(UIProvider.MessageColumns.STARRED,
                            EmailContent.MessageColumns.FLAG_FAVORITE)
    ...
    /**
     * Local-only folders set totalCount < 0; such folders should substitute
     * message count for total count. TODO: IMAP and POP don't adhere to this
     * convention yet so for now we force a few types.
     */
        ...
        if (sFolderListMap == null) {
            sFolderListMap = ProjectionMap
                ...
                .add(UIProvider.FolderColumns.SYNC_STATUS, MailboxColumns.UI_SYNC_STATUS)
                    .add(UIProvider.FolderColumns.LAST_SYNC_RESULT,
                ...
                .add(UIProvider.FolderColumns.LOAD_MORE_URI, uriWithId("uiloadmore"))
                    .add(UIProvider.FolderColumns.HIERARCHICAL_DESC,
                            MailboxColumns.HIERARCHICAL_NAME)
                    .add(UIProvider.FolderColumns.PARENT_URI,
                            "case when " + MailboxColumns.PARENT_KEY
                        + "=" + Mailbox.NO_MAILBOX + " then NULL else " +
                        uriWithColumn("uifolder", MailboxColumns.PARENT_KEY) + " end")
                /**
                     * SELECT group_concat(fromList) FROM (SELECT fromList FROM
                     * message WHERE mailboxKey=? AND flagRead=0 GROUP BY
                ...
                .add(UIProvider.FolderColumns.UNREAD_SENDERS,
                        "(SELECT group_concat(" + MessageColumns.FROM_LIST + ") FROM " +
                                    "(SELECT " + MessageColumns.FROM_LIST + " FROM "
                                    + Message.TABLE_NAME +
    ...
    /**
     * Constructs the map of default entries for accounts. These values can be
     * overridden in {@link #genQueryAccount(String[], String)}.
     */
    private static ProjectionMap getAccountListMap(Context context) {
        if (sAccountListMap == null) {
            ...
            if (!TextUtils.isEmpty(feedbackUri)) {
                // This string needs to be in single quotes, as it will be used
                // as a constant
                // in a sql expression
    ...
    /**
     * Mapping of UIProvider columns to EmailProvider columns for a message's
     * attachments
     */
    private static ProjectionMap getAttachmentMap() {
        if (sAttachmentMap == null) {
            sAttachmentMap = ProjectionMap
                    .builder()
    ...
    /**
     * Generate the SELECT clause using a specified mapping and the original UI
     * projection
     * 
     * @param map the ProjectionMap to use for this projection
     * @param projection the projection as sent by UnifiedEmail
     * @return a StringBuilder containing the SELECT expression for a SQLite
     *         query
     */
    ...
    /**
     * Convenience method to create a Uri string given the "type" of query; we
     * append the type of the query and the id column name (_id)
     * 
    ...
    /**
     * Convenience method to create a Uri string given the "type" of query; we
     * append the type of the query and the passed in column name
     * 
     * @param type the "type" of the query, as defined by our UriMatcher
     *            definitions
    ...
    /**
     * Convenience method to create a Uri string given the "type" of query and
     * the table name to which it applies; we append the type of the query and
     * the fully qualified (FQ) id column (i.e. including the table name); we
     * need this for join queries where _id would otherwise be ambiguous
     * 
     * @param type the "type" of the query, as defined by our UriMatcher
     *            definitions
    ...
    /**
     * Class that holds the sqlite query and the attachment (JSON) value (which
     * might be null)
     */
    ...
    /**
     * Generate the "view message" SQLite query, given a projection from
     * UnifiedEmail
     * 
     * @param uiProjection as passed from UnifiedEmail
     * @return the SQLite query to be executed on the EmailProvider database
     */
    private MessageQuery genQueryViewMessage(String[] uiProjection, String id, boolean loadBody) {
        Context context = getContext();
        ...
        if (msg != null) {
            ...
            if (atts.length > 0) {
                ArrayList<com.blackberry.email.Attachment> uiAtts =
                        new ArrayList<com.blackberry.email.Attachment>();
                for (Attachment att : atts) {
                    // TODO: This code is intended to strip out any inlined
                    // attachments (which
                    // would have a non-null contentId) so that they will not
                    // display at the bottom
                    // along with the non-inlined attachments.
                    // The problem is that the UI_ATTACHMENTS query does not
                    // behave the same way,
                    // which causes crazy formatting.
                    // There is an open question here, should attachments that
                    // are inlined
                    // ALSO appear in the list of attachments at the bottom with
                    // the non-inlined
                    // attachments?
                    // Either way, the two queries need to behave the same way.
                    // As of now, they will. If we decide to stop this, then we
                    // need to enable
                    // the code below, and then also make the UI_ATTACHMENTS
                    // query behave
                    // the same way.
                    //
                    // if (att.mContentId != null && att.getContentUri() !=
                    // null) {
                    // continue;
                    // }
                    com.blackberry.email.Attachment uiAtt =
                            new com.blackberry.email.Attachment();
                ...
                values.put(UIProvider.MessageColumns.ATTACHMENTS, "@?"); // @
                                                                         // for
                                                                         // literal
                attachmentJson = com.blackberry.email.Attachment.toJSONArray(uiAtts);
            }
            if (msg.mDraftInfo != 0) {
             ...
             * This forces the message footer to reload the attachment display
             * when the message is fully loaded.
             */
    ...
    private static void appendConversationInfoColumns(final StringBuilder stringBuilder) {
        // TODO(skennedy) These columns are needed for the respond call for
        // ConversationInfo :(
        // There may be a better way to do this, but since the projection is
        // specified by the
    ...
    /**
     * Generate the "message list" SQLite query, given a projection from
     * UnifiedEmail
     * 
     * @param uiProjection as passed from UnifiedEmail
    ...
    private static String genQueryMailboxMessages(String[] uiProjection, final boolean unseenOnly) {
        ...
        // adding the include flag check for messages being processed by
        // consumers
        sb.append("AND ").append(MessageColumns.FLAG_INCLUDED).append(" = 1 ");
    ...
    /**
     * Generate various virtual mailbox SQLite queries, given a projection from
     * UnifiedEmail
     * 
     * @param uiProjection as passed from UnifiedEmail
    ...
    private Cursor getVirtualMailboxMessagesCursor(SQLiteDatabase db, String[] uiProjection,
        ...
        final int virtualMailboxId = VirtualMailbox.getVirtualMailboxType(getContext(), mailboxId);
        final String[] selectionArgs;
        ...
        // adding only included ones **MFL
        sb.append(MessageColumns.FLAG_INCLUDED).append(" = 1 AND ");

        if (VirtualMailbox.isCombinedMailbox(getContext(), mailboxId)) {
            if (unseenOnly) {
            ...
            selectionArgs = new String[] {
                    Long.toString(VirtualMailbox
                            .getVirtualMailboxAccountId(getContext(), mailboxId))
            };
        }
        switch (VirtualMailbox.getVirtualMailboxType(getContext(), mailboxId)) {
    ...
    /**
     * Generate the "top level folder list" SQLite query, given a projection
     * from UnifiedEmail
     * 
     * @param uiProjection as passed from UnifiedEmail
     * @return the SQLite query to be executed on the EmailProvider database
    ...
    /**
     * Generate the "all folders" SQLite query, given a projection from
     * UnifiedEmail. The list is sorted by the name as it appears in a
    ...
    /**
     * Generate the "recent folder list" SQLite query, given a projection from
     * UnifiedEmail
     * 
     * @param uiProjection as passed from UnifiedEmail
     * @return the SQLite query to be executed on the EmailProvider database
    ...
    private int getFolderCapabilities(EmailServiceInfo info, int type, long mailboxId) {
        // Special case for Search folders: only permit delete, do not try to
        // give any other caps.
        ...
        if (VirtualMailbox.isVirtualMailbox(getContext(), mailboxId)) {
            caps |= UIProvider.FolderCapabilities.IS_VIRTUAL;
        }

        // If we don't know the protocol or the protocol doesn't support it,
        // don't allow moving
        // messages
    ...
    /**
     * Generate a "single mailbox" SQLite query, given a projection from
     * UnifiedEmail
     * 
     * @param uiProjection as passed from UnifiedEmail
     * @return the SQLite query to be executed on the EmailProvider database
    ...
    private String genQueryMailbox(String[] uiProjection, String id) {
        ...
        if (mSearchParams != null && mailboxId == mSearchParams.mSearchMailboxId) {
            ...
            // Make sure we can't get NPE if mailbox has disappeared (the result
            // will end up moot)
            if (mailbox != null) {
                ...
                if (info != null && info.offerLoadMore) {
                    // "load more" is valid for protocols not supporting
                    // "lookback"
                    values.put(UIProvider.FolderColumns.LOAD_MORE_URI,
                            uiUriString("uiloadmore", mailboxId));
                }
                ...
                // The persistent id is used to form a filename, so we must
                // ensure that it doesn't
                // include illegal characters (such as '/'). Only perform the
                // encoding if this
    ...
    public static final String LEGACY_AUTHORITY = "ui.email.blackberry.com";
    private static final Uri BASE_EXTERNAL_URI = Uri.parse("content://" + LEGACY_AUTHORITY);

    private static final Uri BASE_EXTERAL_URI2 = Uri.parse("content://ui.email2.blackberry.com");
    ...
    private static int getCapabilities(Context context, long accountId) {
        ...
        // Account capabilities are based on protocol -- different protocols
        // (and, for EAS,
        ...
        // @NOTE I removed this **MFL
        // If the configuration states that feedback is supported, add that
        // capability
    ...
    /**
     * Generate a "single account" SQLite query, given a projection from
     * UnifiedEmail
     * 
     * @param uiProjection as passed from UnifiedEmail
    ...
    private String genQueryAccount(String[] uiProjection, String id) {
        ...
        // TODO: If uiProjection is null, this will NPE. We should do everything
        // here if it's null.
        ...
        // Set default inbox, if we've got an inbox; otherwise, say initial sync
        // needed
        ...
        if (projectionColumns.contains(
            ...
            // Email doesn't support priority inbox, so always state priority
            // arrows disabled.
    ...
    private Cursor getVirtualMailboxCursor(long mailboxId) {
        ...
        mc.addRow(getVirtualMailboxRow(
                VirtualMailbox.getVirtualMailboxAccountId(getContext(), mailboxId),
                VirtualMailbox.getVirtualMailboxType(getContext(), mailboxId)));
        return mc;
    }

    // TODO: Pass in projection b/10912870
    private Object[] getVirtualMailboxRow(long accountId, int mailboxType) {
        final long id = VirtualMailbox.getVirtualMailboxId(getContext(), accountId, mailboxType);
                ...
                if (accountId == Account.ACCOUNT_ID_COMBINED_VIEW) {
                    ...
                    final int unreadCount = EmailContent.count(getContext(), Message.CONTENT_URI,
                            MessageColumns.MAILBOX_KEY + " IN (SELECT " + MailboxColumns.ID
                                    + " FROM " + Mailbox.TABLE_NAME + " WHERE "
                                    + MailboxColumns.TYPE
                                    + "=" + Mailbox.TYPE_INBOX + ") AND "
            ...
            case Mailbox.TYPE_UNREAD: {
                ...
                final int unreadCount = EmailContent.count(getContext(), Message.CONTENT_URI,
                        accountKeyClause + MessageColumns.FLAG_READ + "=0 AND "
                                + MessageColumns.MAILBOX_KEY + " NOT IN (SELECT "
                                + MailboxColumns.ID
                                + " FROM " + Mailbox.TABLE_NAME + " WHERE " + MailboxColumns.TYPE
                                + "="
        ...
        try {
            ...
            while (accountIdCursor.moveToNext()) {
                ...
                final Cursor accountCursor =
                        db.rawQuery(genQueryAccount(uiProjection, id), new String[] {
                            id
                        });
                try {
    ...
    /**
     * Generate the "attachment list" SQLite query, given a projection from
     * UnifiedEmail
     * 
     * @param uiProjection as passed from UnifiedEmail
     * @param contentTypeQueryParameters list of mimeTypes, used as a filter for
     *            the attachments or null if there are no query parameters
     * @return the SQLite query to be executed on the EmailProvider database
     */
    private static String genQueryAttachments(String[] uiProjection,
        ...
        // filter. IE, "image/" would return "image/png" as well as
        // "image/jpeg".
    ...
    /**
     * Generate the "single attachment" SQLite query, given a projection from
     * UnifiedEmail
     * 
     * @param uiProjection as passed from UnifiedEmail
     * @return the SQLite query to be executed on the EmailProvider database
    ...
    /**
     * Generate the "subfolder list" SQLite query, given a projection from
     * UnifiedEmail
     * 
     * @param uiProjection as passed from UnifiedEmail
     * @return the SQLite query to be executed on the EmailProvider database
    ...
    private static final String COMBINED_ACCOUNT_ID_STRING = Long
            .toString(Account.ACCOUNT_ID_COMBINED_VIEW);

    /**
     * Returns a cursor over all the folders for a specific URI which
    ...
    private Cursor uiVirtualMailboxes(final String id, final String[] uiProjection) {
        ...
        if (id.equals(COMBINED_ACCOUNT_ID_STRING)) {
            mc.addRow(getVirtualMailboxRow(Account.ACCOUNT_ID_COMBINED_VIEW, Mailbox.TYPE_INBOX));
            mc.addRow(getVirtualMailboxRow(Account.ACCOUNT_ID_COMBINED_VIEW, Mailbox.TYPE_STARRED));
            mc.addRow(getVirtualMailboxRow(Account.ACCOUNT_ID_COMBINED_VIEW, Mailbox.TYPE_UNREAD));
        } else {
    ...
    /**
     * Returns an array of the default recent folders for a given URI which is
     * unique for an account. Some accounts might not have default recent
     * folders, in which case an empty array is returned.
     * 
     * @param id account id
    ...
    private Uri[] defaultRecentFolders(final String id) {
        ...
        final String[] idAndType = {
                BaseColumns._ID, UIProvider.FolderColumns.TYPE
        };
    ...
    /**
     * Wrapper that handles the visibility feature (i.e. the conversation list
     * is visible, so any pending notifications for the corresponding mailbox
     * should be canceled). We also handle getExtras() to provide a snapshot of
     * the mailbox's status
     */
    // @NOTE needed to make this a CrossProcessCursorWrapper **MFL
    static class EmailConversationCursor extends CrossProcessCursorWrapper {
        ...
        /**
         * When showing a folder, if it's been at least this long since the last
         * sync, force a folder refresh.
         */
        ...
        public EmailConversationCursor(final Context context, final Cursor cursor,
            ...
            if (mailbox != null) {
                 ...
                 if (mailbox.mUiSyncStatus == EmailContent.SYNC_STATUS_BACKGROUND
                    ...
                    mExtras.putInt(UIProvider.CursorExtraKeys.EXTRA_STATUS,
                     ...
                     if (mailbox.mSyncInterval == 0
                            ...
                            // Consider merging this check with the auto-sync
                            // code in respond.
                ...
                // TODO for virtual mailboxes, we may want to do something
                // besides just fake it
                mExtras.putInt(UIProvider.CursorExtraKeys.EXTRA_ERROR,
            ...
            if (params.containsKey(setVisibilityKey)) {
                ...
                if (visible) {
                    ...
                    if (params.containsKey(
                        ...
                        if (mailbox != null) {
                            // For non-push mailboxes, if it's stale (i.e. last
                            // sync was a while
                            // ago), force a sync.
                            // TODO: Fix the check for whether we're non-push?
                            // Right now it checks
                            // whether we are participating in account sync
                            // rules.
                            if (mailbox.mSyncInterval == 0) {
    ...
    static class AttachmentsCursor extends CursorWrapper {
        ...
        public String getString(int column) {
            if (column == mContentUriIndex) {
                ...
                // Until the package installer can handle opening apks from a
                // content:// uri, for
                // any apk that was successfully saved in external storage,
                // return the
    ...
    /**
     * Converts a mailbox in a row of the mailboxCursor into a row in the
     * supplied {@link MatrixCursor} in the format required for {@link Folder}.
     * As a convenience, the modified {@link MatrixCursor} is also returned.
     * 
     * @param mc the {@link MatrixCursor} into which the mailbox data will be
     *            converted
    ...
    /**
     * Takes a uifolder cursor (that was generated with a full projection) and
     * remaps values for columns that are difficult to generate in the SQL
     * query. This currently includes: - Folder name (due to system folder
     * localization). - Capabilities (due to this varying by account protocol).
     * - Persistent id (due to needing to base64 encode it). - Load more uri
     * (due to this varying by account protocol). TODO: This would be better as
     * a CursorWrapper, rather than doing a copy.
     * 
     * @param inputCursor A cursor containing all columns of
     *            {@link UIProvider.FolderColumns}. Strictly speaking doesn't
     *            need all, but simpler if we assume that.
    ...
    private void remapFolderCursor(final Cursor inputCursor, final MatrixCursor outputCursor,
        ...
        // While we currently could assume the column indicies for
        // UIProvider.FOLDERS_PROJECTION
        // and therefore avoid the calls to getColumnIndex, this at least tries
        // to future-proof a
        // bit.
        // Note that id and type MUST be present for this function to work
        // correctly.
        ...
        // Build the return cursor. We iterate over all rows of the input cursor
        // and construct
        ...
        while (inputCursor.moveToNext()) {
            ...
            for (int i = 0; i < uiProjection.length; i++) {
                // Find the index in the input cursor corresponding the column
                // requested in the
                // output projection.
                final int index = inputCursor.getColumnIndex(uiProjection[i]);
                if (index == -1) {
                    // We don't have this value, so put a blank in the output
                    // and move on.
                    builder.add(null);
                    continue;
                }
                final String value = inputCursor.getString(index);
                // remapped indicates whether we've written a value to the
                ...
                if (nameColumn == index) {
                    ...
                    // Blank the load more uri for account types that don't
                    // offer it.
                    // Note that all account types permit load more for search
                    // results.
                    builder.add(null);
                    remapped = true;
                } else {
                ...
                // If the above logic didn't write some other value to the
                // output, use the value
    ...
    /**
     * Returns a {@link String} from Resources corresponding to the
     * {@link UIProvider.FolderType} requested.
     * 
     * @param folderType {@link UIProvider.FolderType} value for the folder
     * @param defaultName a {@link String} to use in case the
     *            {@link UIProvider.FolderType} provided is not a system folder.
     ...
     * {@link UIProvider.FolderType} equivalent.
     * 
    ...
    /**
     * We need a reasonably full projection for getFolderListCursor to work, but
     * don't always want to do the subquery needed for
     * FolderColumns.UNREAD_SENDERS
     * 
     * @param uiProjection The projection we actually want
     * @return Full projection, possibly with or without
     *         FolderColumns.UNREAD_SENDERS
     */
     ...
     * 
     * @param match the UriMatcher match for the original uri passed in from
     *            UnifiedEmail
     * @param uri the original uri passed in from UnifiedEmail
     * @param uiProjection the projection passed in from UnifiedEmail
     * @param unseenOnly <code>true</code> to only return unseen messages (where
     *            supported)
    ...
    private Cursor uiQuery(int match, Uri uri, String[] uiProjection, final boolean unseenOnly) {
        ...
        switch (match) {
            case UI_ALL_FOLDERS:
                ...
                if (id.equals(COMBINED_ACCOUNT_ID_STRING)) {
                    // There's no real mailboxes, so just return the virtual
                    // ones
                    final Cursor vc = uiVirtualMailboxes(id, uiProjection);
                    vc.setNotificationUri(context.getContentResolver(), notifyUri);
                    c = vc;
                    ...
                    c = rawc;
                    // @NOTE merge cursor was causing issues, for now removing
            ...
            case UI_FULL_FOLDERS: {
                ...
                c = db.rawQuery(genQueryAccountAllMailboxes(folderProjection), new String[] {
                    id
                });
                c = getFolderListCursor(c, Long.valueOf(id), uiProjection);
                notifyUri =
            ...
            case UI_RECENT_FOLDERS:
                c = db.rawQuery(genQueryRecentMailboxes(uiProjection), new String[] {
                    id
                });
            ...
            case UI_SUBFOLDERS: {
                // We need a full projection for getFolderListCursor
                final String[] folderProjection = folderProjectionFromUiProjection(uiProjection);
                c = db.rawQuery(genQuerySubfolders(folderProjection), new String[] {
                    id
                });
                c = getFolderListCursor(c, Mailbox.getAccountIdForMailbox(context, id),
                        uiProjection);
                // Get notifications for any folder changes on this account.
                // This is broader than
                // we need but otherwise we'd need for every folder change to
                // notify on all relevant
            ...
            case UI_MESSAGES:
                ...
                if (folder == null) {
                    ...
                    // TODO: Make callers of this query handle null cursors
                    // instead b/10819309
                ...
                if (VirtualMailbox.isVirtualMailbox(getContext(), mailboxId)) {
                    ...
                    c = db.rawQuery(
                            genQueryMailboxMessages(uiProjection, unseenOnly), new String[] {
                                id
                            });
                }
                ...
                break;
            case UI_MESSAGE:
                // just testing to see if query parameter will be ok
                String loadBodyQString = uri
                        .getQueryParameter(UIProvider.LOAD_BODY_QUERY_PARAMETER);
                boolean loadBody = false;
                if (loadBodyQString != null && loadBodyQString.equalsIgnoreCase("true")) {
                    loadBody = true;
                }

                MessageQuery qq = genQueryViewMessage(uiProjection, id, loadBody);
                String sql = qq.query;
                String attJson = qq.attachmentJson;
                // With attachments, we have another argument to bind
                if (attJson != null && loadBody) {
            ...
            case UI_ATTACHMENTS:
                ...
                break;
            case UI_ATTACHMENT:
                c = db.rawQuery(genQueryAttachment(uiProjection, id), new String[] {
                    id
                });
            ...
            case UI_FOLDER:
                ...
                if (VirtualMailbox.isVirtualMailbox(getContext(), mailboxId)) {
                    ...
                    c = db.rawQuery(genQueryMailbox(uiProjection, id), new String[] {
                        id
                    });
                ...
                break;
            case UI_ACCOUNT:
                c = db.rawQuery(genQueryAccount(uiProjection, id), new String[] {
                    id
                });
                notifyUri = UIPROVIDER_ACCOUNT_NOTIFIER.buildUpon().appendPath(id).build();
                break;
            case UI_CONVERSATION:
                c = db.rawQuery(genQueryConversation(uiProjection), new String[] {
                    id
                });
                break;
        }
    ...
    /**
     * Convert a UIProvider attachment to an EmailProvider attachment (for
     * sending); we only need a few of the fields
     * 
     ...
     */
    // TODO(pwestbro): once the Attachment contains the cached uri, the second
    // parameter can be
    // removed
    // TODO(mhibdon): if the UI Attachment contained the account key, the third
    // parameter could
    // be removed.
    private static Attachment convertUiAttachmentToAttachment(
            com.blackberry.email.Attachment uiAtt, String cachedFile, long accountKey) {
    ...
    private Mailbox createMailbox(long accountId, int mailboxType) {
        ...
        // If these already exist (from old Email app), they will have touch
        // times
        switch (mailboxType) {
            case Mailbox.TYPE_DRAFTS:
    ...
    /**
     * Given an account name and a mailbox type, return that mailbox, creating
     * it if necessary
     * 
     * @param accountId the account id to use
     * @param mailboxType the type of mailbox we're trying to find
     * @return the mailbox of the given type for the account in the uri, or null
     *         if not found
     */
    ...
    /**
     * Given a mailbox and the content values for a message, create/save the
     * message in the mailbox
     * 
     * @param mailbox the mailbox to use
     * @param extras the bundle containing the message fields
     * @return the uri of the newly created message TODO(yph): The following
     *         fields are available in extras but unused, verify whether they
     *         should be respected: - UIProvider.MessageColumns.SNIPPET -
    ...
    private Uri uiSaveMessage(Message msg, Mailbox mailbox, Bundle extras) {
        ...
        final List<com.blackberry.email.Attachment> uiAtts =
                com.blackberry.email.Attachment.fromJSONArray(
        ...
        for (com.blackberry.email.Attachment uiAtt : uiAtts) {
            final Uri attUri = uiAtt.uri;
            if (attUri != null && attUri.getAuthority().equals(EmailContent.AUTHORITY)) {
                // If it's one of ours, retrieve the attachment and add it to
                // the list
                ...
                if (att != null) {
                    // We must clone the attachment into a new one for this
                    // message; easiest to
                    // use a parcel here
                    ...
                    p.recycle();
                    // Clear the messageKey (this is going to be a new
                    // attachment)
                    attClone.mMessageKey = 0;
                    // If we're sending this, it's not loaded, and we're not
                    // smart forwarding
                ...
                // Cache the attachment. This will allow us to send it, if the
                // permissions are
                // revoked.
                final String cachedFileUri =
                        AttachmentUtilities.cacheAttachmentUri(context, uiAtt, attachmentFds);

                // Convert external attachment to one of ours and add to the
                // list
        ...
        // @NOTE good stop to do the checking for Interceptors
        // flip included flag,
        // send data to interceptor
        // update according to interceptor response
        // May need to see if it is OUTBOX
        boolean bEnableInterceptorLogic = false;
        boolean isOutBox = mailbox.mType == Mailbox.TYPE_OUTBOX;
        if (isOutBox
                && hasRegisteredInterceptReceivers(context,
                        EMAIL_OUTGOING_MESSAGE_INTERCEPT_INTENT_ACTION)) {
            msg.mFlagIncluded = 0;
            bEnableInterceptorLogic = true;
        } else {
            msg.mFlagIncluded = 1;
        }
     ...
     // Save it or update it...
        if (!msg.isSaved()) {
            ...
            ops.add(ContentProviderOperation.newDelete(Body.CONTENT_URI)
                    .withSelection(Body.MESSAGE_KEY + "=?", new String[] {
        ...
        if (bEnableInterceptorLogic) {
            // MessageConsumerTimeManager.getInstance().addOutgoingTimeoutMessageTask(msg,
            // mailbox, this, context);
           this.sendOutgoingInterceptBroadcastIntent(msg, mailbox, extras);
        } else {
            notifyUIMessage(msg.mId);
        }

        if (isOutBox && !bEnableInterceptorLogic) {
            startSync(mailbox, 0);
            final long originalMsgId = msg.mSourceKey;
            if (originalMsgId != 0) {
                final Message originalMsg = Message.restoreMessageWithId(
                        context, originalMsgId);
                // If the original message exists, set its forwarded/replied to
                // flags
                if (originalMsg != null) {
    ...
    private Uri uiSendDraftMessage(final long accountId, final Bundle extras) {
        ...
        // Kick observers
        notifyChange(Mailbox.CONTENT_URI, null);
        return messageUri;
    }
    ...
    /**
     * Update the timestamps for the folders specified and notifies on the
     * recent folder URI.
     * 
    ...
    /**
     * Updates the recent folders. The values to be updated are specified as
     * ContentValues pairs of (Folder URI, access timestamp). Returns nonzero if
     * successful, always.
     * 
    ...
    private ContentValues convertUiMessageValues(Message message, ContentValues values) {
        ...
        for (String columnName : values.keySet()) {
            ...
            if (columnName.equals(UIProvider.ConversationColumns.STARRED)) {
                ...
                // Skip this column, as the folders will also be specified the
                // RAW_FOLDERS column
                ...
                // Email's conversation info is generated, not stored, so just
                // ignore this update
            } else {
    ...
    /**
     * Add an undo operation for the current sequence; if the sequence is newer
     * than what we've had, clear out the undo list and start over
     * 
    ...
    /**
     * Does post processing of a Message Update.  The processing includes updating the DomainMessageProviders data
     ...
     * @param values
     */
    private void handleMessageUpdated(Context context, final Uri uri, final String messageId,
            final ContentValues values){
        //make call to update agains DomainMessageProvider
        MessageProviderUtilities.updateMessage(context,messageId,values);
        //do notifications call
        handleMessageUpdateNotifications(uri, messageId , values);
    }
    ...
    private int pickFolder(Uri uri, int type, int headerId) {
        ...
        // For push imap, for example, we want the user to select the trash
        // mailbox
        Cursor ac = query(uiUri("uiaccount", acctId), UIProvider.ACCOUNTS_PROJECTION,
                null, null, null);
        try {
            if (ac.moveToFirst()) {
                final com.blackberry.email.Account uiAccount =
                        new com.blackberry.email.Account(ac);

                // @TODO make a propert intent Filter action **MFL
                // Intent intent = new Intent(context,
                // FolderPickerActivity.class);
                Intent intent = new Intent();
                intent.setAction("com.blackberry.email.action.FOLDER_PICKER_EDIT");
    ...
    private Cursor uiUndo(String[] projection) {
        ...
        if (!mLastSequenceOps.isEmpty()) {
            try {
                ...
                // Not sure if UI wants it, but I'm making a cursor of convo
                // uri's
                MatrixCursor c = new MatrixCursorWithCachedColumns(
                        new String[] {
                            UIProvider.ConversationColumns.URI
                        },
    ...
    private void notifyUIConversationMailbox(long id) {
        ...
        if (mailbox.mType == Mailbox.TYPE_INBOX) {
            notifyUI(UIPROVIDER_CONVERSATION_NOTIFIER,
                    VirtualMailbox.combinedMailboxId(getContext(), Mailbox.TYPE_INBOX));
        }
        notifyWidgets(id);
    }
    ...
    /**
     * Notify about a folder update. Because folder changes can affect the
     * conversation cursor's extras, the conversation must also be notified
     * here.
     * 
     * @param folderId the folder id to be notified
     * @param accountId the account id to be notified (for folder list
     *            notification).
     */
    private void notifyUIFolder(final String folderId, final long accountId) {
        ...
        notifyUI(UIPROVIDER_FOLDER_NOTIFIER,
                VirtualMailbox.getVirtualMailboxId(getContext(), Account.ACCOUNT_ID_COMBINED_VIEW,
                        Mailbox.TYPE_INBOX));
        notifyUI(UIPROVIDER_FOLDERLIST_NOTIFIER, Account.ACCOUNT_ID_COMBINED_VIEW);
     ...
     * @return an android.accounts.Account for this account, or null if we can't
     *         load it.
     */
    ...
    /**
     * Create an android.accounts.Account object for an emailAddress/protocol
     * pair.
     * 
     * @param context A {@link Context}.
     ...
     */
    public  static android.accounts.Account getAccountManagerAccount(final Context context,
    ...
    private void updateAccountSyncInterval(final long accountId, final ContentValues values) {
        ...
        // Only positive values of sync interval indicate periodic syncs. The
        // value is in minutes,
     ...
     * @param mailboxId The mailbox id we want to sync (or one of the special
     *            constants in {@link Mailbox}).
     * @param deltaMessageCount If we're requesting a load more, the number of
     *            additional messages to sync.
     */
    ...
    private static boolean isMessageBodyEmpty(final String html, final String text) {
    ...
    private void fetchMessageBodyIfNeeded(Cursor c, Uri uri, String[] projection) {
        // Check if the projection contains a request for the body
        List<String> projList = Arrays.asList(projection);
        boolean checkHtml = projList.contains(BodyColumns.HTML_CONTENT);
        boolean checkText = projList.contains(BodyColumns.TEXT_CONTENT);
        if (!checkHtml && !checkText) {
            return;
        }
        ...
        Long messageId = null;
        if (c.moveToFirst()) {
            // could also just find the index of the project and use that in the
            // cursor gets, but for now this feels
            // safter **MFL
            if (checkHtml) {
                int indexHtml = c.getColumnIndex(BodyColumns.HTML_CONTENT);
                if (indexHtml > 0) {
                    html = c.getString(indexHtml);
                }
            }
            if (checkText) {
                int indexText = c.getColumnIndex(BodyColumns.TEXT_CONTENT);
                if (indexText > 0) {
                    text = c.getString(indexText);
                }
            }
            if (!isMessageBodyEmpty(html, text)) {
                return;
            }
            LogUtils.i(TAG, "fetchMessageBodyIfNeeded: body does NOT exist fetching");
            messageId = c.getLong(Body.CONTENT_MESSAGE_KEY_COLUMN);
        } else {
            return;
        }
        ...
        final Mailbox mailbox = Mailbox.restoreMailboxWithId(context, msg.mMailboxKey);
        if (mailbox == null) {
            return;
        }
        ...
        final Bundle extras = RequestSyncParameters.createDefaultExtras();
        extras.putBoolean(RequestSyncParameters.FETCH_MESSAGE, true);
        extras.putString(RequestSyncParameters.MAILBOX_SYNC_ID, mailbox.mServerId);
        extras.putString(RequestSyncParameters.MAILBOX_SYNC_KEY, mailbox.mSyncKey);
        extras.putString(RequestSyncParameters.MESSAGE_SYNC_ID, msg.mServerId);
        extras.putLong(RequestSyncParameters.MESSAGE_ID, msg.mId);

        LogUtils.i(TAG, "Calling requestSync account:%s, bundle:%s", account.toString(),
                extras.toString());
        ContentResolver.requestSync(account, EmailContent.AUTHORITY, extras);
    }
     ...
     *            additional messages to sync.
     */
    public void startSync(final Mailbox mailbox, final int deltaMessageCount) {
        final android.accounts.Account account = getAccountManagerAccount(mailbox.mAccountKey);
    ...
    // Number of additional messages to load when a user selects "Load more..."
    // in POP/IMAP boxes
    ...
    private Cursor uiFolderLoadMore(final Mailbox mailbox, boolean apiSearch) {
        if (mailbox == null)
            return null;
        if (mailbox.mType == Mailbox.TYPE_SEARCH) {
            ...
            if(apiSearch) {
                ApiRemoteSearch searchRequest = mApiSearches.get(mailbox.mId);
                if(searchRequest == null)
                    return null;
                searchParams = searchRequest.mSearchParams;
            } else {
                searchParams = mSearchParams;
            }
            searchParams.mOffset += SEARCH_MORE_INCREMENT;
            runSearchQuery(getContext(), mailbox.mAccountKey, mailbox.mId, searchParams);
        } else {
    ...
    private static class ApiRemoteSearch {
        public SearchParams mSearchParams;
        public int mCallerGid;
        public long mMailboxId;
    }

    private static final String SEARCH_MAILBOX_SERVER_ID = "__search_mailbox__";
    private static final String API_SEARCH_MAILBOX_SERVER_ID = "__api_search_mailbox__";
    private SearchParams mSearchParams;
    private LongSparseArray<ApiRemoteSearch> mApiSearches = new LongSparseArray<ApiRemoteSearch>();

    /**
     * Returns the search mailbox for the specified account, creating one if
     * necessary
     * 
     * @param apiSearch if searching from API then creating a new mailbox
     * @return the search mailbox for the passed in account
     */
    private Mailbox getSearchMailbox(long accountId, boolean apiSearch) {
        Context context = getContext();
        Mailbox m = apiSearch ? null : Mailbox.restoreMailboxOfType(context, accountId, Mailbox.TYPE_SEARCH);
        if (m == null) {
            m = new Mailbox();
            m.mAccountKey = accountId;
            m.mServerId = apiSearch ? API_SEARCH_MAILBOX_SERVER_ID : SEARCH_MAILBOX_SERVER_ID;
            m.mFlagVisible = false;
            m.mDisplayName = apiSearch ? API_SEARCH_MAILBOX_SERVER_ID : SEARCH_MAILBOX_SERVER_ID;
            m.mSyncInterval = 0;
            m.mType = Mailbox.TYPE_SEARCH;
            m.mFlags = apiSearch ? Mailbox.FLAG_HOLDS_MAIL : (Mailbox.FLAG_HOLDS_MAIL | Mailbox.FLAG_API_REMOTE_SEARCH);
            m.mParentKey = Mailbox.NO_MAILBOX;
            if(apiSearch) {
                // set the live query flag to show an hour glass
                m.mUiSyncStatus = UIProvider.SyncStatus.LIVE_QUERY;
            }
            m.save(context);
        }
        return m;
    ...
    private void runSearchQuery(final Context context, final long accountId,
            final long searchMailboxId, final SearchParams searchParams) {
    ...
    // This handles an initial search query. More results are loaded using
    // uiFolderLoadMore.
    private Cursor uiSearch(Uri uri, String[] projection) {
        ...
        Mailbox searchMailbox = getSearchMailbox(accountId, false);
        ...
        if (mSearchParams.mOffset == 0) {
            // TODO: This conditional is unnecessary, just two lines earlier we
            // created
        ...
        // Start the search running in the background
        runSearchQuery(context, accountId, searchMailboxId, mSearchParams);
    ...
    private String apiRemoteSearch(long accountId, String filter, int limit) {
        try {
            final Context context = getContext();

            Mailbox inbox = Mailbox.restoreMailboxOfType(getContext(), accountId, Mailbox.TYPE_INBOX);
            if (inbox == null) {
                throw new Exception("In api remote search, inbox doesn't exist for account " + accountId);
            }

            Mailbox searchMailbox = getSearchMailbox(accountId, true);
            final long mailboxId = searchMailbox.mId;
            String folderUri = MessageProviderUtilities.buildFolderId(mailboxId);

            // sync the new folder with Domain Provider

            SearchParams searchParams = new SearchParams(inbox.mId, filter, mailboxId);
            if (limit > 0)
                searchParams.mLimit = limit;
            // Start the search running in the background
            runSearchQuery(context, accountId, searchMailbox.mId, searchParams);

            ApiRemoteSearch ars = new ApiRemoteSearch();
            ars.mMailboxId = mailboxId;
            ars.mSearchParams = searchParams;
            ars.mCallerGid = Binder.getCallingUid();
            mApiSearches.append(mailboxId, ars);

            return folderUri;

        } catch (Exception ex) {
            LogUtils.w(TAG, "Failed to query for messages", ex);
        }
        return null;
    }
    ...
    private int uiDeleteAccount(Uri uri) {
        ...
        try {
            ...
            // @TODO uncomment when code refactor is doen **AJE
            // SecurityPolicy.getInstance(context).reducePolicies();
            System.out.println("TODO: SecurityPolicy.getInstance(context).reducePolicies();");
            // @TODO uncomment when code refactor is doen **MFL
    ...
    private int uiDeleteAccountData(Uri uri) {
        ...
        // will remove from PIM AccountProvider --NEED TO FIX DEPENDANCIES
        // if(account.mPimAccountId != -1){
        // context.getContentResolver().delete(
        // ContentUris.withAppendedId(AccountContract.Account.CONTENT_URI,account.mPimAccountId),null,null);
        // }
        return 1;
    }
    ...
    private static void deleteAccountData(Context context, long accountId) {
        // We will delete PIM data, but by the time the asynchronous call to do
        // that happens,
        // the account may have been deleted from the DB. Therefore we have to
        // get the email
        ...
        if (emailAddress != null) {
            ...
            if (service != null) {
                try {
                    service.deleteAccountPIMData(accountId, emailAddress);
    ...
    public static final String NOTIFY_WIDGETS = "com.blackberry.mail.action.notification.NOTIFY_WIDGETS";

    private void notifyWidgets(long mailboxId) {
        Context context = getContext();

        Intent intent = new Intent(NOTIFY_WIDGETS);
        intent.putExtra("mailboxId", mailboxId);
            ...
            mDelayedSyncHandler = new Handler(getContext().getMainLooper(), new Callback() {
                ...
                public boolean handleMessage(android.os.Message msg) {
                    synchronized (mDelayedSyncRequests) {
                        ...
                        // TODO: It's possible that the account is deleted by
                        // the time we get here
                        // It would be nice if we could validate it before
                        // trying to sync
